-- USE [Nova_hls]
-- GO
-- /****** Object:  View [dbo].[gv_SS_Dogodek_View]    Script Date: 12.1.2017. 14:56:35 ******/
SET ANSI_NULLS ON
GO
...
/*--------------------------------------------------------------------------------------------------------
 ...
 13.01.2017 g_tomislav; MR 37035 - fixed on site SIF_BANK into '000'
--------------------------------------------------------------------------------------------------------*/

ALTER VIEW [dbo].[gv_SS_Dogodek_View]
AS
SELECT D.id_dog, D.id_cont, D.id_kupca, D.dat_vnosa, D.vnesel, D.rok, D.izvedeno, D.opis_dog, D.to_do, D.kont_oseba, D.tel_ko, 
       ...
       (dbo.gfn_Xchange('000', PP.saldo + PP.se_neto, '000', getdate())) AS obligo,
       (dbo.gfn_Xchange('000', D.dolg_dat_d, '000', getdate())) AS dolg_dat_d,
       (dbo.gfn_Xchange('000', D.plac_dogov, '000', getdate())) AS plac_dogov,
       (dbo.gfn_Xchange('000', PP.saldo, '000', getdate())) AS saldo,
       (dbo.gfn_Xchange('000', PP.se_neto, '000', getdate())) AS se_neto,
       (dbo.gfn_Xchange('000', PP.bod_findavek, '000', getdate())) AS bod_findavek
