IF EXISTS(SELECT * FROM sys.views WHERE name = 'gv_SS_Dogodek_View') DROP VIEW [dbo].[gv_SS_Dogodek_View]
GO
/*--------------------------------------------------------------------------------------------------------
 ...
 28.12.2016 Blaz; MR 60707 - fixed the exchange
 26.01.2017 Tomislav - fixed on site
--------------------------------------------------------------------------------------------------------*/
...
SELECT D.id_dog, D.id_cont, D.id_kupca, D.dat_vnosa, D.vnesel, D.rok, D.izvedeno, D.opis_dog, D.to_do, D.kont_oseba, D.tel_ko, 
       ...
       --(dbo.gfn_Xchange('000', PP.saldo + PP.se_neto, PP.id_tec, getdate())) AS obligo,
	   PP.saldo + PP.se_neto AS obligo,
       --(dbo.gfn_Xchange('000', D.dolg_dat_d, PP.id_tec, getdate())) AS dolg_dat_d,
	   D.dolg_dat_d AS dolg_dat_d,
       --(dbo.gfn_Xchange('000', D.plac_dogov, PP.id_tec, getdate())) AS plac_dogov,
	   D.plac_dogov AS plac_dogov,
       --(dbo.gfn_Xchange('000', PP.saldo, PP.id_tec, getdate())) AS saldo,
	   PP.saldo AS saldo,
       --(dbo.gfn_Xchange('000', PP.se_neto, PP.id_tec, getdate())) AS se_neto,
	   PP.se_neto AS se_neto,
       --(dbo.gfn_Xchange('000', PP.bod_findavek, PP.id_tec, getdate())) AS bod_findavek
	   PP.bod_findavek AS bod_findavek
 ...
 INNER JOIN dbo.users AS UV ON D.vnesel = UV.username
  ...
  LEFT JOIN (SELECT DS.id_cont,
              ...
              GROUP BY DS.id_cont, N.tip_knjizenja, n.ol_na_nacin_fl, DS.ID_TEC) AS PP ON D.id_cont = PP.id_cont
