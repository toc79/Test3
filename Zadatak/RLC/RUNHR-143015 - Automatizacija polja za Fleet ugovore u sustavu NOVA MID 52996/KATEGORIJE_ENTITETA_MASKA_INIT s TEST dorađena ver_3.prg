	...
	* TKU Tip kalkulacije: Otvorena or Zatvorena
	...
	IF !GF_NULLOREMPTY(_ef_kategorije_TKU.id_kategorije_tip) && for active entity
		...
		IF lcIdPosrednik == "FLT" 
			...
			ELSE
				IF GF_NULLOREMPTY(_ef_kategorije_TKU.id_kategorije_sifrant) OR _ef_kategorije_TKU.id_kategorije_sifrant != _ef_sifrant_TKU_zatvorena.id_kategorije_sifrant
					...
					OBVESTI("Za TKU Tip kalkulacije se automatski postavila vrijednost 'Zatvorena' jer ugovor ima unesenog Posrednika = FLT " +lcPosrednikNaziv + ".")
				ENDIF
			ENDIF
		ENDIF
	ENDIF
	...
	* TKGU Tip kalkulacije guma: Otvorena or Zatvorena
	...
	IF !GF_NULLOREMPTY(_ef_kategorije_TKGU.id_kategorije_tip) && for active entity
		...
		IF lcIdPosrednik == "FLT" 
			...
			ELSE
				IF GF_NULLOREMPTY(_ef_kategorije_TKGU.id_kategorije_sifrant) OR _ef_kategorije_TKGU.id_kategorije_sifrant != _ef_sifrant_TKGU_Limitirana.id_kategorije_sifrant
					...
					lcPosrednikNaziv = ALLT(NVL(GF_SQLEXECScalarNull("select dbo.gfn_StringToFox(value) from dbo.general_register where id_register = 'P_POSREDNIK' and id_key = 'FLT'"), ""))
					OBVESTI("Za TKGU Tip kalkulacije guma se automatski postavila vrijednost 'Limitirana' jer ugovor ima unesenog Posrednika = FLT " +lcPosrednikNaziv + ".")
				ENDIF
			ENDIF
		ENDIF
	ENDIF
