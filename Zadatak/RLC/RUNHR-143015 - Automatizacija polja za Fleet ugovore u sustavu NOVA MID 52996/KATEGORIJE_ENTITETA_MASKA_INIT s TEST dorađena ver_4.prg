...
****************
* 21.08.2024 g_tomislav MID 52996 - created 
****************
IF loForm.txtEntiteta.Value == "POGODBA" THEN
	...
	* TKU Tip kalkulacije: Otvorena or Zatvorena
	...
	IF !GF_NULLOREMPTY(_ef_kategorije_TKU.id_kategorije_tip) && for active entity
		...
		IF lcIdPosrednik == "FLT" 
			...
			ELSE
				IF GF_NULLOREMPTY(_ef_kategorije_TKU.id_kategorije_sifrant) && only when it is empty 
					...
					lcPosrednikNaziv = ALLT(NVL(GF_SQLEXECScalarNull("select dbo.gfn_StringToFox(value) from dbo.general_register where id_register = 'P_POSREDNIK' and id_key = 'FLT'"), ""))
					lcMessage = lcMessage +("Za TKU Tip kalkulacije se automatski postavila vrijednost 'Zatvorena' jer ugovor ima unesenog Posrednika = FLT " +lcPosrednikNaziv + ".") +gce
				ENDIF
			ENDIF
		ENDIF
		...
		IF lcIdPosrednik == "DOBF" 
			...
			ELSE
				IF GF_NULLOREMPTY(_ef_kategorije_TKU.id_kategorije_sifrant) && only when it is empty 
					...
					lcPosrednikNaziv = ALLT(NVL(GF_SQLEXECScalarNull("select dbo.gfn_StringToFox(value) from dbo.general_register where id_register = 'P_POSREDNIK' and id_key = 'DOBF'"), ""))
					lcMessage = lcMessage +("Za TKU Tip kalkulacije se automatski postavila vrijednost 'Otvorena' jer ugovor ima unesenog Posrednika = DOBF " +lcPosrednikNaziv + ".") +gce
				ENDIF
			ENDIF
		ENDIF
	...
	* TKGU Tip kalkulacije guma: Otvorena or Zatvorena
	...
	IF !GF_NULLOREMPTY(_ef_kategorije_TKGU.id_kategorije_tip) && for active entity
		...
		IF lcIdPosrednik == "FLT" 
			...
			ELSE
				IF GF_NULLOREMPTY(_ef_kategorije_TKGU.id_kategorije_sifrant) && only when it is empty 
					...
					lcPosrednikNaziv = ALLT(NVL(GF_SQLEXECScalarNull("select dbo.gfn_StringToFox(value) from dbo.general_register where id_register = 'P_POSREDNIK' and id_key = 'FLT'"), ""))
					lcMessage = lcMessage +("Za TKGU Tip kalkulacije guma se automatski postavila vrijednost 'Limitirana' jer ugovor ima unesenog Posrednika = FLT " +lcPosrednikNaziv + ".") +gce
				ENDIF
			ENDIF
		ENDIF
	ENDIF
