...
SELECT Ugovor, Tip_financ 
	...
	, d.znesek AS Buduæa_glavnica --bod_neto_lpod
	...
	, Dat_izrade_konaènog_obraèuna --kl.zacetek 	
FROM (
	SELECT a.id_pog Ugovor, nacin_leas Tip_financ 
		...
		, kl.zacetek AS Dat_izrade_konaènog_obraèuna
	--, ru.* --, a.*
		, a.id_tec
	FROM dbo.pogodba a
	...
	LEFT JOIN (SELECT id_cont, id_tec, znp_saldo_brut_all, bod_neto_lpod
					...
					, poknj_nezap_debit_brut_all - poknj_nezap_neto_LPOD - poknj_nezap_robresti_LPOD AS OSTALO --OSTALO: buduæi neto i PPMV se veæ nalazi u SKUPAJ tj. u buduæoj glavnici
