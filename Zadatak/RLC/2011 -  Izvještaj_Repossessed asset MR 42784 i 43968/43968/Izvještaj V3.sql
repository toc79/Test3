...
12.05.2020 Tomislav MID 43968 (RLC 2090) - promjena logike;
*/
...
--Kandidati
--provjera na produkciji -- skoro 63 kandidata
/*
select id_cont 
from dbo.pogodba a 
where exists (select * from dbo.dokument where id_obl_zav in ('RA') and status_akt = 'A')
and exists (select * from dbo.dokument where id_obl_zav in ('RE') and status_akt = 'A')
*/
--a.id_pog, a.id_kupca, a.pred_naj, a.aneks, a.Status, a.nacin_leas, a.id_tec, a.id_vrste
...
FROM (
	SELECT a.id_pog Ugovor, nacin_leas Tip_financ 
		...
		--Control_poin_1
		, CASE WHEN dbo.gfn_Nacin_leas_HR(a.nacin_leas) = 'OL' AND (isnull(otv.RA_otvoren, 0) > 0 OR isnull(otv.RA_zatvoren, 0) > 0) and isnull(rpg08.ima_rpg_08, 0) > 0 and otv.RE_otvoren > 0 
		...
		--Control_poin_2		
		, CASE WHEN isnull(otv.RA_otvoren, 0) > 0 and isnull(otv.RE_otvoren, 0) > 0 THEN dateadd(dd, 365, ra.zacetek) ELSE NULL END AS Control_poin_2 --otvoreni RE dokument. Ako Ugovor ima otvoreni RA dokument -> ROK 365 dana od preuzimanja/vraćanja (podatak iz 1.otvorenog RA dokumenta (aktivan/neaktivan) polje „Početak“ + 365 dana) -> Control poin 2
		...
		, CASE WHEN isnull(otv.RA_zatvoren, 0) > 0 and isnull(otv.RE_otvoren, 0) > 0 THEN 
			...
			ELSE NULL END AS Control_poin_3 --otvoreni RE dokument. Ako Ugovor ima zatvoreni RA dokument, a otvoreni RE dokument ROK 90 dana od dana unovčenja (podatak iz RE dokumenta (aktivan) polje „Vrijedi do“+ 90 dana) -> Control poin 3
