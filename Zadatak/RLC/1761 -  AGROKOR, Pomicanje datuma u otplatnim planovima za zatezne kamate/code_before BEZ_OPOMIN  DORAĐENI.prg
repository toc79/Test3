...
DO case
	CASE lnOdg = 2	&& 7 Trenutnega
		...
		SELECT za_opom	
		IF oznacen = .f. or GF_NULLOREMPTY(dok_opom) or !GF_NULLOREMPTY(ddv_id) or INLIST(id_za_opom_type, 6, 8)
		...
		* "next 1"
	CASE lnOdg = 1	&& 6 Vse
		obj_ReportSelector.obj_reportPrinter.rep_scope = "FOR oznacen = .T. and !GF_NULLOREMPTY(dok_opom) AND GF_NULLOREMPTY(ddv_id) and !INLIST(id_za_opom_type, 6, 8)"
		...
		**list = GF_CreateDelimitedList("cur_za_opom","id_kupca", "", ",",.t.)
		...
		SELECT za_opom
		LOCATE FOR oznacen = .T. AND !GF_NULLOREMPTY(dok_opom) AND GF_NULLOREMPTY(ddv_id) AND !INLIST(id_za_opom_type, 6, 8)
