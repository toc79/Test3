...
text to lcSelect
	...
	where ID_SELECTION in (	616, --POG_SSOFT_RLC - Ugovor - Stimulsoft
							...
							663 -- Punomoć za preuzimanje COC dokumenta - Word    
							) 
endtext
...
* 14.12.2018 g_tomislav MR 41455; added transform in format "9999999999", this is needed when cursor is created with syntax "INTO CURSOR" so field can have fixed and sufficient length ( otherwise field length is set considering first record and that can lead to information loss, if field length of other records is at least one character larger then first record)
* PONUDBA_SSOFT_RLC in not listed above so it is unnecessary   
...
* 04.05.2017 g_tomislav MR 37977 (RLC 1707) - isključivanje određenih ispis
* 14.01.2019 g_barbarak MR 43901 (RLC 2089) - isključivanje DOD_POG_SSOFT_RLC
* 24.02.2020 g_tkovacev	MR 43354 (RLC 2051) - iz delete-a maknut POG_SSOFT_RLC" AND RF_TIP_POG(nacin_leas) != "OL", sada se ispisuju svi tipovi leasinga
