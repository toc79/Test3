...
-- 04.01.2015 Andrej; MID 54014 - added insert fields kategorija4, kategorija5, kategorija6 into table dokument
-- 16.03.2016 Andrej; MID 55930 - added insert fields id_kupca and id_pov_dok
------------------------------------------------------------------------------------------------------------------------------
...
kategorija4, kategorija5, kategorija6, id_kupca, id_pov_dok
)
SELECT 
	null, 			-- DAT_1OP
	null, 			-- DAT_2OP
	...
	END,			-- kategorija3
	CASE 
		WHEN exists (SELECT * FROM @documentFieldsNotTransferFromOld_tab WHERE id = 'kategorija4') THEN null
		ELSE d.kategorija4
	END,			-- kategorija4
	CASE 
		WHEN exists (SELECT * FROM @documentFieldsNotTransferFromOld_tab WHERE id = 'kategorija5') THEN null
		ELSE d.kategorija5
	END,			-- kategorija5
	CASE 
		WHEN exists (SELECT * FROM @documentFieldsNotTransferFromOld_tab WHERE id = 'kategorija6') THEN null
		ELSE d.kategorija6
	END,			-- kategorija6
	CASE 
		WHEN exists (SELECT * FROM @documentFieldsNotTransferFromOld_tab WHERE id = 'id_kupca') THEN null
		ELSE d.id_kupca
	END,			-- id_kupca
	CASE 
		WHEN exists (SELECT * FROM @documentFieldsNotTransferFromOld_tab WHERE id = 'id_pov_dok') THEN null
		ELSE d.id_pov_dok
	END				-- id_pov_dok
