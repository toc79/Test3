...
SELECT po.id_pon, nl.naziv, nl.tip_knjizenja, po.naziv_kup, nl.ima_opcijo, nl.leas_kred, nl.finbruto, 
...
--KRAJ DORADE
	...
	CASE 
		WHEN nl.tip_knjizenja = 1 and CHARINDEX(nl.nacin_leas,@zakup_nek) = 0 THEN 'OL'
		WHEN nl.tip_knjizenja = 2 and nl.finbruto = 0 and nl.leas_kred = 'L' THEN 'FF'
		WHEN nl.tip_knjizenja = 2 and nl.finbruto = 1 and nl.leas_kred = 'L' THEN 'F1'
		WHEN nl.tip_knjizenja = 1 and CHARINDEX(nl.nacin_leas,@zakup_nek) > 0 THEN 'OZ'
		ELSE 'ZP' 
	END as tip_leas, 
