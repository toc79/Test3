...
--20.7.2018 g_tomislav; MID 39253 - added criteria db_name and new fields
...
SELECT b.sif_terj, pp.id_cont, pp.st_dok, pp.datum_dok
INTO #PLANP 
FROM  {9}.dbo.planp pp
...
SELECT id_cont, {9}.dbo.gfn_GetOpcSt_dok(id_cont, nacin_leas) AS OpcSt_dok --, nova_hls.dbo.gfn_Nacin_leas_HR(nacin_leas) AS tip_leas
INTO #OPCST_DOKS
...
FROM #PLANP pp
WHERE NOT EXISTS (SELECT OpcSt_dok FROM #OpcSt_doks WHERE OpcSt_dok IS NOT NULL AND OpcSt_dok = pp.st_dok)
AND sif_terj = 'LOBR' -- za svaki slučaj ako OL ima dva otkupa 
GROUP BY pp.id_cont 

-- Broj rate iz plana otplate
SELECT id_cont, count(*) st_obrok 
INTO #PLANP_ST_OBROK
FROM #PLANP WHERE sif_terj = 'LOBR' 
GROUP BY id_cont
...
x.opcija + case when nl.odstej_var = 1 then x.varscina else 0 end ost_vred,
...
--	nl.tip_leas+'-'+oc.id_grupe as lpi, 
	...
	*/
	...
	oc.dat_podpisa,	oc.pred_naj, oc.id_dob, oc.kategorija, oc.opcija, oc.varscina,
	...
	LEFT JOIN (Select @id_oc_report as id_oc_report, e.id_kupca, e.dat_eval, 
					...
					e.coll_ratin, e.oall_ratin, y.value as model_naziv
...
left join (
	Select id_cont, sum(debit_dom) debit_dom, sum(kredit_dom) kredit_dom
	From dbo.oc_lsk
	Where id_oc_report = @id_oc_report And konto in ('120933', '120936', '120951', '120945', '116931', '116934')
	Group by id_cont
)  ifrs9_1 on x.id_cont = ifrs9_1.id_cont 
left join (
	Select id_cont, sum(debit_dom) debit_dom, sum(kredit_dom) kredit_dom
	From dbo.oc_lsk
	Where id_oc_report = @id_oc_report And konto in ('120934', '120937', '120952', '120946', '116932', '116935')
	Group by id_cont
)  ifrs9_2 on x.id_cont = ifrs9_2.id_cont 
left join (
	Select id_cont, sum(debit_dom) debit_dom, sum(kredit_dom) kredit_dom
	From dbo.oc_lsk
	Where id_oc_report = @id_oc_report And konto in ('120935', '120938', '120953', '120947', '121927', '121928', '116933', '116936', '116938', '116940', '120940', '120942', '120949', '120955', '121929', '121930', '120939', '120941', '116937', '116939', '120954', '120948')
	Group by id_cont
