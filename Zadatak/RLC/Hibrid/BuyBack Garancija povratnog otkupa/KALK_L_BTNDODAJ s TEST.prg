...
if (allt(lookup(nacini_l.tip_knjizenja, _newponudba.nacin_leas, nacini_l.nacin_leas)) = '1' AND ATC(allt(_newponudba.nacin_leas),lcZakupi) = 0) OR lookup(nacini_l.ol_na_nacin_fl, _newponudba.nacin_leas, nacini_l.nacin_leas) = .t. THEN

	llol_na_nacin_fl = lookup(nacini_l.ol_na_nacin_fl, _newponudba.nacin_leas, nacini_l.nacin_leas)

	local lnbuyback, lcXml

	lnbuyback = GF_GET_NUMBER("Unesite iznos ugovorenog otkupa, buyback/ostalo", _newponudba.opcija, "", .F., "Buyback")

	
	IF lnbuyback <= 0 and llol_na_nacin_fl = .t. THEN
		=obvesti("Kalkulacija OF mora sadržavati buyback!")
		use in _newponudba
	...
	IF lnbuyback != 0 and llol_na_nacin_fl = .f. THEN
		=obvesti("Potrebno je odabrati tip leasinga OF!")
		use in _newponudba
