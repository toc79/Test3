...
** 27.06.2023 g_tomislav MID 50889 - ZO dokument okvira se sada nalazi u posebnom šifrantu RLC_DOKUMENTI_OKVIRA te je kreiran #dokumenti_okvira
		...
		* Lista ugovora
		...
		IF EMPTY(lcList) AND (!GF_NULLOREMPTY(dokument.id_frame) OR !GF_NULLOREMPTY(dokument.id_cont)) && da li se radi o krovnom dokumentu ili ugovoru vezanom na okvir
			TEXT TO lcSQL36380_4 NOSHOW
				...
				set @lista = (Select cast(value as [text]) as value from dbo.GENERAL_REGISTER Where ID_REGISTER = 'RLC Reporting list' and id_key = 'RLC_DOKUMENTI_OKVIRA' and neaktiven = 0)
				...
				from
					...
					left join (select a.id_cont, fr.id_frame, count(*) br_zaduznica
								...
								--and a.ID_OBL_ZAV = 'ZO' 
								and exists (select * from #dokumenti_okvira where id = a.id_obl_zav)
								group by a.id_cont,fr.id_frame) broj_zad on broj_zad.ID_CONT=fr.id_cont and fr.id_frame=broj_zad.id_frame 
					) a
