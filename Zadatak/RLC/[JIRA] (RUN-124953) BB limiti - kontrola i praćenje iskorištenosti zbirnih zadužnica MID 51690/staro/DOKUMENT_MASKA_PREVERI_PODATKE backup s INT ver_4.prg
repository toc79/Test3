...
***************************************************
** 27.02.2024. g_tomislav MID 51690 - check if sum of ZE documents exceeds value of frame document ZT
...
IF !GF_NULLOREMPTY(lnId_krov_dok) AND dokument.id_obl_zav == "ZE"  && if not entered, then it is null
	...
	*IF RECCOUNT("_ef_dokument_ZE_ZT") > 0 THEN &&* shows data used in calculation only if there is another ZE document
	...
	SELECT ZT_vrednost AS ZT_Vrijednost, ZT_id_tec, id_dokum AS ZE_Br_dok, id_pog AS ZE_Ugovor, ZE_vrednost AS ZE_Vrijednost FROM _ef_dokument_ZE_ZT ;
	UNION ALL ;
	SELECT ZT_vrednost AS ZT_Vrijednost, ZT_id_tec, lnId_dokum51690 AS ZE_Br_dok, lcId_pog AS ZE_Ugovor, lnVrednost AS ZE_Vrijednost FROM _ef_dokument_ZE_ZT ;
	UNION ALL ;
	SELECT ZT_vrednost AS ZT_Vrijednost, ZT_id_tec, 0 AS ZE_Br_dok, 'Suma' AS ZE_Ugovor, lnZE_VrednostUkupno AS ZE_Vrijednost FROM _ef_dokument_ZE_ZT 
	_VFP.DATATOCLIP(,,3)
	*ENDIF
