...
**///////////////////////////////
** 09.03.2023 g_tomislav MID 49406

IF INLIST(fakture.id_terj, "2D", "2E", "2H") 
	llIs_allowed = GF_SQLEXECSCALAR("select dbo.gfn_ContractCanOfferPriorRedemp(" +ALLT(STR(fakture.id_cont)) +", getdate()) as is_allowed")
	IF ! llIs_allowed
		POZOR("Potraživanja 2D, 2E, 2H se ne mogu izdati za ugovore koji u posebnostima imaju zaustavljen automatizam 'Priprema ponude za prijevremeni zaključak/opomenu do'!")
		SELECT cur_extfunc_error
		REPLACE ni_napaka WITH .F. IN cur_extfunc_error
	ENDIF
ENDIF
