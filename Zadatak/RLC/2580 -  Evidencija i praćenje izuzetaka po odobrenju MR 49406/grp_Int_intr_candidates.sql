...
-- strm  
@par_strm_enabled bit = 0,  
@par_strm_value varchar(500) = ''  
  
AS  
BEGIN  
DECLARE @cmd nvarchar(max), @cmd_where nvarchar(max), @top varchar(5), @E varchar(10), @is_ext_func bit, @zap_obr char(1), @id_kupca varchar(8), @param_xml varchar(max), @xml_tmp varchar(max)  
...
begin  
    ...
    set @param_xml = @param_xml + '<akt_od>' + @par_dat_akt_od + '</akt_od>' + '<akt_do>' + @par_dat_akt_do + '</akt_do>'  
end  
  
IF @par_nacinleas_enabled = 1  
BEGIN  
    set @xml_tmp = (select id as nacin_leas from dbo.gfn_GetTableFromList(@par_nacinleas_nacinleas) FOR XML path('nacini_l_row'))  
    set @param_xml = @param_xml + @xml_tmp  
...
begin  
    set @xml_tmp = (select id as id_strm from dbo.gfn_GetTableFromList(@par_strm_value) FOR XML path('strm_row'))  
    set @param_xml = @param_xml + @xml_tmp  
end  
set @param_xml = @param_xml + '</params>'  
...
SET @cmd = '  
    SELECT ' + @top + '  
        ...
        1 AS tip_izracuna  
    INTO #inter_temp  
    FROM dbo.gfn_get_fin_intr_candidates1(' + @zap_obr + ',' + @id_kupca + ',''' + @param_xml + ''')  
    WHERE 1 = 1 ' + @cmd_where  
...
BEGIN  
    SET @cmd = @cmd + @E + '  
        ...
        @par_strm_enabled bit, @par_strm_value varchar(500)'',   
        ...
        @par_strm_enabled, @par_strm_value  
    '  
END  
