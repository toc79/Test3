...
--where exists (select top 1 opis1 from dbo.dokument where id_cont = nf.id_cont and id_obl_zav = 'DU' order by id_dokum desc )
--order by datum_dok desc
...
print char(10)+char(10)+'vvvvvvvvvvvvvv  ORIGINALNI s Dokument vvvvvvvvvvvvvv' +char(10)
...
print char(10)+char(10)+'vvvvvvvvvvvvvv  S gv_Dokument umjesto Dokument  vvvvvvvvvvvvvv' +char(10)
...
INNER JOIN dbo.dav_stop h ON a.id_dav_st = h.id_dav_st
LEFT JOIN (SELECT a.id_cont, /*a.id_dokum,*/ a.opis1
		FROM dbo.gv_Dokument a 
		INNER JOIN (SELECT MAX(id_dokum) AS id FROM dbo.gv_Dokument WHERE id_obl_zav = 'DU' GROUP BY id_cont) b ON a.id_dokum = b.id
			) dok on a.id_cont = dok.id_cont
...
print char(10)+char(10)+'vvvvvvvvvvvvvv  OUTER APPLY s gv_Dokument vvvvvvvvvvvvvv' +char(10)
...
--		INNER JOIN (SELECT MAX(id_dokum) AS id FROM dbo.dokument /*with (Index(IX_DOKUMENT_ic), index(IX_DOKUMENT_IOZ))*/ WHERE id_obl_zav = 'DU' GROUP BY id_cont) b ON a.id_dokum = b.id
--			) dok on a.id_cont = dok.id_cont
outer apply (select top 1 opis1 from dbo.gv_Dokument where id_obl_zav = 'DU' and id_cont = a.id_cont order by id_dokum desc) dok --on a.id_cont = dok.id_cont RADI SPORIJE OD LEFT JOINA, EXECUTIN PLAN JA 49% 
LEFT JOIN dbo.P_KONTAKT kon on a.id_kupca = kon.id_kupca and kon.ID_VLOGA = 'IV' AND kon.NEAKTIVEN = 0
LEFT JOIN dbo.P_KONTAKT konp on a.id_kupca = konp.id_kupca and konp.ID_VLOGA = 'DP' AND konp.NEAKTIVEN = 0
LEFT JOIN dbo.P_KONTAKT konXW on a.id_kupca = konXW.id_kupca and konXW.ID_VLOGA = 'XW' AND konXW.NEAKTIVEN = 0
INNER JOIN dbo.rac_out ra on a.ddv_id = ra.ddv_id
...
print char(10)+char(10)+'vvvvvvvvvvvvvv  OUTER APPLY s dokument vvvvvvvvvvvvvv' +char(10)
