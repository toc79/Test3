...
lnOdg = rf_msgbox("Odabir dokumenta za podešavanje polja IMA","Želite li podesiti više dokumenata ili trenutni dokument?","Više","Jedan","Poništiti")
...
DO CASE
	CASE lnOdg = 1	&& Više
		...
		Select .T. as Oznacen, id_dokum, id_obl_zav, opis, ima, id_cont, lcKup as id_kupca From Dokument Where id_obl_zav # 'IE' Group by id_obl_zav INTO CURSOR _PATTERN
		GF_DataPreview("_PATTERN", "", "frmPripDok", "Odaberite dokumente za koje želite prijenos polja IMA")
		USE IN _PATTERN
	CASE lnOdg = 2	&& Jedan
		IF DOKUMENT.ID_OBL_ZAV != 'IE' THEN
			lcID = Dokument.ID_DOKUM
			Select .T. as Oznacen, id_obl_zav, ima, id_cont, lcKup as id_kupca From Dokument Where ID_DOKUM = lcID INTO CURSOR _PATTERN
