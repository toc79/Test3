		...
		begin 
			...
			begin 
				...
				InvoiceNote = case when @tip_leas in ('F1','ZP') 
								...
								+ case when isnull(kat_naziv_odjela.val_string, '') != '' then iif(InvoiceNote != '', char(10), '') +'Naziv odjela: ' +rtrim(kat_naziv_odjela.val_string) + '.' else '' end,
				...
				outer apply (
					Select top 1 ke.val_string
					From dbo.kategorije_entiteta ke
					inner join dbo.kategorije_tip kt on ke.id_kategorije_tip = kt.id_kategorije_tip and kt.entiteta = 'POGODBA'
					where kt.sifra = 'NAZIV_ODJELA'
					and a.ID_CONT = cast(ke.id_entiteta as int)
				) kat_naziv_odjela -- on pog.ID_CONT = kat_naziv_odjela.id_cont and a.ID_KUPCA = p.id_kupca
...
begin 
		...
		'394' as InvoiceType, 
		a.ddv_date as InvoiceDeliveryDate,
		cast(case when isnull(kat_naziv_odjela.val_string, '') != '' then 'Naziv odjela: ' +rtrim(kat_naziv_odjela.val_string) else '' end, as varchar(max)) as InvoiceNote, -- Ubl 1024 znakova, UblEn nema ograničenja
		rtrim(a.izdal) as InvoicePersonIssued,
		a.id_kupca as InvoiceCustomerId,
		a.dav_stev as InvoiceCustomerOIB,
		...
		left join (
			Select a.id_cont, a.ddv_id, b.val_string
			From najem_fa a
			inner join (
				Select cast(a.id_entiteta as int) as id_cont, a.val_string
				From dbo.kategorije_entiteta a
				inner join dbo.kategorije_tip b on a.id_kategorije_tip = b.id_kategorije_tip and b.entiteta = 'POGODBA' and b.neaktiven = 0
				where b.sifra = 'NAZIV_ODJELA') b on a.id_cont = b.id_cont 
			where isnull(b.val_string, '') != ''
		) kat_naziv_odjela on z.ddv_id = kat_naziv_odjela.ddv_id 
