...
exec dbo.grp_Int_intr_candidates 
    ?st_pog_e, ?st_pog_v, 
    ?sif_part_e, ?sif_part_v, 
    ?akt_e, ?akt_v1, ?akt_v2, 
    ?nacinleas_e, ?nacinleas_v, 
    ?status_akt_e, ?status_akt_v1, ?status_akt_v2, 
    ?kand_e, 
    ?aneks_e, ?aneks_v1, ?aneks_v2, 
    ?from_to_e, ?from_to_v1, ?from_to_v2
	
	
SELECT count(*) FROM dbo.ext_func WHERE id_ext_func = 'INTERCALARY_INTERESTS' AND id_ext_func_type = 'SQL_RF' AND inactive = 0 and len(rtrim(code)) > 0

SELECT * FROM dbo.ext_func WHERE id_ext_func = 'INTERCALARY_INTERESTS'

SELECT CAST(pol_je_1ob AS char(1)) FROM dbo.nastavit

select CONVERT(char(8), (SELECT TOP 1 datum_zak FROM dbo.nastavit), 112)
...
ALTER PROCEDURE [dbo].[grp_Int_intr_candidates]
...
ALTER FUNCTION [dbo].[gfn_get_fin_intr_candidates1](@zap_obr int, @id_kupca char(6))
returns table as
return(
