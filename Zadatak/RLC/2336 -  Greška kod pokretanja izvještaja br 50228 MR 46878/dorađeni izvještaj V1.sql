...
select cast(1 as bit) as selected,
	...
	par.emso, 
	par.naz_kr_kup, 
	{1}.dbo.gfn_stringtofox(gr.value) as oall_ratin_desc
	, cast(case when bpm.id_kupca is not null then 1 else 0 end as bit) as ima_nezavrsenu_instancu
from {1}.dbo.p_eval ez
inner join (
		select id_kupca
		from {1}.dbo.p_eval
		where eval_type='Z'
...
left join {1}.dbo.partner par on ez.id_kupca = par.id_kupca
left join {1}.dbo.general_register gr on rtrim(ez.oall_ratin) = rtrim(gr.id_key) and gr.id_register = 'OVALL_RAT' and gr.val_char='Z'
