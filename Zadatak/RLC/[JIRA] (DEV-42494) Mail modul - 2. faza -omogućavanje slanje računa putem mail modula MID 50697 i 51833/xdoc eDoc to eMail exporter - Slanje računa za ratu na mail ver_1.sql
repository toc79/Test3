...
/*
-- SQL SHOW AKO NEŽE RUČNO POKRETATI; TAJ SELECT JE NEPOTREBAN ZAPRAVO NITI MOGU RUČNO POKRETATI JER PATH ZA DATOTEKU VIŠE NE SADRŽI DOKUMENTE NA TOJ LOKACIJI VEĆ U ARCHIVE FOLDERU (moglo bi se podesiti i ručno pokretanje) TAKO DA ĆU TO MAKNUTI
-- 24.08.2023 g_tomislav MID 50697 - created;
...
Poštovani,
<BR>
<BR>u privitku se nalazi račun za ratu.
<BR>
<BR>S poštovanjem,
...
--where cast(a.id as varchar(30)) in (SELECT b.doc_id FROM dbo.xdoc_document_tmp b WHERE b.session_id = @session_id AND b.filter = 1)
--Group by a.id_kupca, d.mail_to -- zbog digitalnog potpisa ide jedan po jedan mail. kasnije se može vidjeti da se grupira po batch/saršama od npr. 10 koristeći row_number

--second table
--impossible condition to get the second table empty - it is used for report rendering
select 
...
select cast(eef.id as varchar(40)) as doc_id,  -- bilo varchar(10)
