...
--ne trebaju sve kolone; može se staviti #datoteke kao podselect ?
...
INTO #final
From #main a
left join dbo.najem_fa b on a.document_id = b.ddv_id And a.id_edoc_doctype = 'Invoice' And a.izvor_racuna = 'NAJEM_FA' --NIJE POTREBNO ?? ili ipak je potrebno zbog id_terj
...
-- dovoljan je jedan join na dbo.reports_log jer postoji izravna veza s edoc_exported files što se može napraviti već u #MAIN koristeći gv_EDOC_EXPORTED_FILES_CHANNELS
Left join (Select id_report, id_object_edoc 
			...
			--iznda: može ih biti više istih document_id tako da ovo nije točno i duplicirati će redove
Left join (Select id_report, id_object_edoc 
			From dbo.reports_log 
