...
--28.11.2023 g_tomislav - dorada logike za kolonu email. Optimizacija selecta
...
select top 20 convert(varchar(30), dbo.gfn_GetDatePart(date_prepared), 104) as date_prepared, export_id, COUNT(*) as rows from dbo.EDOC_EXPORTED_FILES where date_prepared >='20231001' group by dbo.gfn_GetDatePart(date_prepared), export_id order by rows desc
...
DECLARE @export_id varchar(100)
SET @export_id = '1EFACF3C-91FB-423F-9308-FC847BCCD590' --'04031216-E0A3-410C-A77B-46FC0F621779'

select * 
    , case when id_edoc_doctype = 'Invoice' Then dbo.gfn_GetInvoiceSource(document_id) else null end as izvor_racuna
into #main
from (
    select id_file, doc_date_prepared as date_prepared, doc_document_id as document_id, doc_id_edoc_doctype as id_edoc_doctype, doc_id_kupca as id_kupca, id_reports_log, id_report, id_edoc_channel, convert(int, iif(was_ignored = 0, 1, 0)) as was_ignored
    from dbo.gv_edoc_exported_files_channels eefc
...
pivot 
    (
    sum(was_ignored)
    for id_edoc_channel in ([EDOC_EX1], [EDOC_EX2], [EDOC_EX3], [EDOC_EX4], [EDOC_EX5], [EDOC_EX6], [EDOC_EXPORT_FINA]) --, [EDOC_EX5], [EDOC_EX6] seu isključeni jer za tu statistiku se uzimaju podaci iz xdoc_document
...
--select id_file, doc_date_prepared as date_prepared, doc_document_id as document_id, doc_id_edoc_doctype as id_edoc_doctype, doc_id_kupca as id_kupca, id_reports_log, id_report, id_edoc_channel, convert(int, iif(was_ignored = 0, 1, 0)) as was_ignored
--    from dbo.gv_edoc_exported_files_channels eefc
--    where id_file = 1169347

--select * from dbo.EDOC_EXPORTED_FILES_CHANNELS where id_file = 1169347

--select * from #main order by id_file



Select a.*,
    CASE 
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'VARSCINA_SSOFT_RLC' THEN 'Jamčevina'
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'OBVREG_SSOFT' THEN 'Dopis za registraciju'
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'OBV_O1_SSOFT_RLC' THEN 'Obavijest o neplaćenim potraživanjima'
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'OBV_OPC_SSOFT_RLC' THEN 'Obavijest za otkup'
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'KU_DNEV_SSOFT_RLC' THEN 'Dnevnik kupaca LSK'
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'GL_K_DNEV_SSOFT_RLC' THEN 'Dnevmnik GK'
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'OTHER2GK_SSOFT_RLC' THEN 'Dnevnik prijenosa iz FA'	
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(id_report)) = 'BPM_SCORING_VIEW' THEN 'BPM scoring view'
	    ...
	    WHEN a.id_edoc_doctype = 'GuarRemind' AND LTRIM(RTRIM(id_report)) = 'OPOMJAM_SSOFT_RLC' THEN 'Obavijest dodatnim jamcima o opomeni'
	    WHEN a.id_edoc_doctype = 'GuarRemind' AND LTRIM(RTRIM(id_report)) = 'OBV_POR_SSOFT_RLC' THEN 'Obavijest jamcu o opomeni'
	    WHEN a.id_edoc_doctype = 'Contract' AND LTRIM(RTRIM(id_report)) = 'NAL_PL_SSOFT_RLC' THEN 'Nalog za plaćanje'
	    WHEN a.id_edoc_doctype = 'Approval' THEN 'Odobrenje financiranja'
	    WHEN a.id_edoc_doctype = 'Contract' AND LTRIM(RTRIM(id_report)) = 'PLANP_SSOFT_RLC' THEN 'Plan otplate'
	    WHEN a.id_edoc_doctype = 'Contract' AND LTRIM(RTRIM(id_report)) = 'PROM_OTPL_FL_SSOFT_RLC' THEN 'Dopis o promj. sadr. otpl. tablice FL'
	    WHEN a.id_edoc_doctype = 'Contract' AND LTRIM(RTRIM(id_report)) = 'PROM_OTPL_OL1_SSOFT_RLC' THEN 'Obavijest o izmjeni visine obroka OL'
	    WHEN a.id_edoc_doctype = 'Contract' AND LTRIM(RTRIM(id_report)) = 'ZAH_UK_REG_SSOFT_RLC' THEN 'Zahtjev za ukidanjem registracije'
	    WHEN a.id_edoc_doctype = 'Invoice' 
		    ...
		    ELSE  a.id_edoc_doctype +' ' + LTRIM(RTRIM(id_report))
		END END AS tip,
    CASE WHEN pk.id_kupca IS NOT NULL THEN 1 ELSE 0 END AS xw_racun
    , convert(int, case when xdd57.doc_id is not null then 1 else 0 end) as email_LOBR
	, convert(int, case when xdd54.doc_id is not null then 1 else 0 end) as email_OBV_IND
INTO #final
From #main a
...
left join dbo.xdoc_document xdd54 on a.id_file = xdd54.doc_id and xdd54.id_xdoc_template = 54 and xdd54.date_exported >= a.date_prepared
left join dbo.xdoc_document xdd57 on a.id_file = xdd57.doc_id and xdd57.id_xdoc_template = 57 and xdd57.date_exported >= a.date_prepared
...
Select tip, count(*) as broj
	...
	, sum(EDOC_EXPORT_FINA) as fina
    --, sum([EDOC_ARH]) as arh, sum([EDOC_EXPORT_FINA]) as fina, SUM(email) as email
From #final
Group by tip
