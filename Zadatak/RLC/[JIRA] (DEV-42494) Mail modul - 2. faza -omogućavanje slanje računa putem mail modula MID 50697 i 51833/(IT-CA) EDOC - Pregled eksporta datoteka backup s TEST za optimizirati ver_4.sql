...
--28.11.2023 g_tomislav MID 50397 i 50697 - dorada logike za kolonu email. Optimizacija #main i #final 
...
from (
	...
	--where doc_date_prepared >='20230301'
    ) as SourceTable
pivot 
    (
...
Select a.*,
    CASE 	-- moglo bi se maknuti id_edoc_doctype za uvjete gdje se gleda/provjerava id_report, Invoice treba detaljnije provjeriti
	    ...
	    WHEN a.id_edoc_doctype = 'General' AND LTRIM(RTRIM(a.id_report)) = 'GL_K_DNEV_SSOFT_RLC' THEN 'Dnevmnik GK'
		...
		ELSE ltrim(rtrim(ps.rep_name)) +' ' +ltrim(rtrim(a.id_report)) +' ' +a.id_edoc_doctype --XXXX
		--END ELSE  a.id_edoc_doctype +' ' + ltrim(rtrim(a.id_report)) +' ' +ltrim(rtrim(ps.rep_name)) --XXXX
		END AS tip,
    CASE WHEN pk.id_kupca IS NOT NULL THEN 1 ELSE 0 END AS xw_racun
	, convert(int, case when xdd54.doc_id is not null and a.EDOC_EX5 > 0 then 1 else 0 end) as email_OBV_IND
    , convert(int, case when xdd57.doc_id is not null and a.EDOC_EX6 > 0 then 1 else 0 end) as email_LOBR
...
outer apply ( -- obavijesti s DDV_ID u napomeni (ne provjerava se za sada FINA ID (ident_stevilka))
			select id_rep_ind  
			from dbo.rep_ind ri 
			inner join #main m on m.izvor_racuna = 'NAJEM_FA' and charindex(m.document_id, ri.opombe) > 0
