...
-- set @id = '83787'
-- set @OriginalFileName = 'Contract_83787_2023_12_07_11_39_46_456.pdf'
-- set @DocType = 'Contract'
-- set @ReportName = 'PLANP_SSOFT_RLC'
...
declare @is_for_fina bit, @is_for_mail bit
...
begin
	...
	if @is_for_fina = 0
		set @is_for_mail = (select cast(count(*) as bit) 
							from dbo.rac_out ro
							where ro.ddv_id = @id
							and exists (select * 
...
begin
	...
	if @is_for_fina = 0
		set @is_for_mail = (Select cast(count(*) as bit)
						From dbo.zbirniki a
						inner join dbo.rac_out ro on a.ddv_id = ro.ddv_id 
						inner join dbo.partner c on ro.id_kupca = c.id_kupca
						Where a.id_zbirnik = @id
						and exists (select * 
									From dbo.p_kontakt a 
									inner join dbo.p_kontakt_vloga b on a.id_vloga = b.id_vloga 
									Where a.neaktiven = 0 
...
-- 23.08.2023 g_tomislav MID 50697 - added pdf_sign
-- 11.12.2023 g_tomislav MID 51814 - bugfix - pogodba was replaced with gv_PogodbaAll for Contract and General
-- 20.12.2023 g_tomislav MID 51833 - added @is_for_mail (excluding LOBR Invoices from channels PCK (edoc.filter_field) and Ne PCK (edoc.not_print))
-- 29.02.2024 g_tomislav MID 51835 - excluding TaxChngIx from channels: PCK (edoc.filter_field), Ne PCK (edoc.not_print) and [edoc.dms] for those already printed in FAK_LOBR_SSOFT_RLC
-- 20.06.2024 g_majam MID 52469 - added @is_for_mail (excluding InvoiceCum from channels PCK (edoc.filter_field) and Ne PCK (edoc.not_print)
-- 29.08.2024 g_tomislav MID 52944 - changed InvoiceNote for SPR_DDV for old_ddv_d >= '2024-01-01'	
-- 03.09.2024 g_tomislav MID 52838 - changed InvoiceNote for FINA invoice @source = 'POGODBA' (invoice for activation)
-- 24.09.2024 g_tomislav MID 52846 - added NAZIV_ODJELA in InvoiceNote
...
begin
	...
	Select naz_kr_kup as [partner_title],
		...
		CASE WHEN @is_for_fina = 1 then '0'
				when @is_for_mail = 1 and v.sif_terj = 'LOBR' then '0'
...
begin
	...
	From dbo.za_regis a 
	--Inner join dbo.partner b on a.id_kupca = b.id_kupca
    Inner join dbo.gv_PogodbaAll p on a.id_cont = p.id_cont
...
begin
	...
	Select '0014' as [tip_dokumenta],
		...
		a.id_kupca as partner_id,
		a.naz_kr_kup as partner_title,
		a.id_cont as contract_id, 
		RTRIM(a.id_pog) as contract_number,
		0 as pdf_sign
	From dbo.gv_PogodbaAll a 
	--Inner join dbo.partner b on a.id_kupca = b.id_kupca
...
begin
	...
	left join dbo.najem_fa nf on left(a.opombe, 11) = nf.ddv_id --already printed in FAK_LOBR_SSOFT_RLC
...
BEGIN
	...
	SELECT '0033' AS [tip_dokumenta],
		...
		pog.naz_kr_kup AS partner_title,
	...
	inner join dbo.gv_PogodbaAll pog ON odg.id_cont = pog.id_cont
...
IF @DocType = 'Contract' AND @ReportName = 'OBV_ISTEK_SSOFT_RLC'
BEGIN
	SELECT '0033' AS [tip_dokumenta],
		'1' AS [edoc.dms],
		CASE WHEN @eom_blockade = 0 THEN '0' ELSE '1' End [edoc.filter_field],
		'0' AS [edoc.for_web],
		'0' AS [edoc.not_print],
		RTRIM(pog.id_kupca) + '_0033_' + rtrim(@Id) +'.pdf' AS print_centar_name,
		pog.id_kupca AS partner_id,
		pog.naz_kr_kup AS partner_title,
		pog.id_cont as contract_id,					   
		RTRIM(LTRIM(pog.id_pog)) AS contract_number,
		0 as pdf_sign
	FROM dbo.gv_PogodbaAll pog
	--JOIN dbo.partner par ON pog.id_kupca = par.id_kupca
	inner join dbo.reports_log b on b.edoc_file_name = @OriginalFileName and cast(pog.id_cont as varchar(100)) = b.id_object_edoc
...
begin
	...
	case when @is_for_fina = 1 then '0' else '1' end as [edoc.dms],
	CASE 
		when @is_for_fina = 1 then '0'
		when @is_for_mail = 1 then '0'
	    WHEN @eom_blockade = 0 THEN '0' 
		when f.id_kupca is not null then '0'								  
		ELSE '1' End as [edoc.filter_field],
	'0' as [edoc.for_web],
	...
	left join dbo.p_kontakt f on b.id_kupca = f.id_kupca And f.id_vloga = 'XP' AND f.neaktiven = 0																   
	Where cast(z.id_zbirnik as varchar(100)) =  @id and @DocType = 'InvoiceCum'

end

if @DocType = 'Contract' and @ReportName = 'NAROC_SSOFT_RLC'
begin

	Select '0050' as [tip_dokumenta],
		'1' as [edoc.dms],
		'0' as [edoc.filter_field],
		'0' as [edoc.for_web],
		'0' as [edoc.not_print],
		'Narudžbenica_' + RTRIM(a.id_pog) + '_' + '0050' + '_' + RTRIM(@Id) + '.pdf' As print_centar_name,
		a.id_kupca as partner_id,
		a.naz_kr_kup as partner_title,
		a.id_cont as contract_id, 
		RTRIM(a.id_pog) as contract_number,
...
Begin
		...
		begin 
			...
			begin 
				...
				InvoiceNote = case when @tip_leas in ('F1','ZP') 
								...
								+ case when isnull(kat_naziv_odjela.val_string, '') != '' then 'Naziv odjela: ' +rtrim(kat_naziv_odjela.val_string) + '.' else '' end,
				...
				left join (
						...
						where b.sifra = 'NAZIV_ODJELA'
					) kat_naziv_odjela on a.id_cont = kat_naziv_odjela.id_cont --and a.ID_KUPCA = p.id_kupca
		...
		begin
				...
				InvoiceNote = case when b.id_dav_op = 'NM' then 'Vrijednost objekta leasinga: '+ dbo.gfn_gccif(d.brez_davka) + ' ' + rtrim(n.dom_valuta) else 'Vrijednost objekta leasinga bez PDV-a: '+ dbo.gfn_gccif(d.debit_neto) + ' ' + rtrim(n.dom_valuta) end +
							  ...
							  + case when e.debit > 0 then CHAR(10) + 'SPECIFIKACIJA PREDUJMOVA' + CHAR(10) + ltrim(rtrim(b.kk_memo)) + ' - Osnova: ' + dbo.gfn_gccif(e.debit_neto) + ' ' + rtrim(n.dom_valuta) + ', PDV: ' + dbo.gfn_gccif(e.debit_davek) + ' ' + rtrim(n.dom_valuta) + ', Ukupno: ' + dbo.gfn_gccif(e.debit) + ' ' + rtrim(n.dom_valuta) + CHAR(10) + 'Razlika po Rn. - Osnova: '+ dbo.gfn_gccif(a.debit_neto) + ' ' + rtrim(n.dom_valuta)+ ', PDV: ' + dbo.gfn_gccif(a.debit_davek) + ' ' + rtrim(n.dom_valuta) + case when a.neobdav > 0 and vo.se_regis = '*' then ', PPMV: ' + dbo.gfn_gccif(a.neobdav) + ' ' + rtrim(n.dom_valuta) else '' end + ', Ukupno: ' + case when vo.se_regis = '*' then dbo.gfn_gccif(a.debit+a.neobdav) else dbo.gfn_gccif(a.debit) end + ' ' + rtrim(n.dom_valuta) +'.' else '' end + CHAR(10)
							+ 'Objekt leasinga ostaje u vlasništvu ' + rtrim(@p_podjetje) + ' do konačne otplate svih obveza po Ugovoru o leasingu broj ' + rtrim(b.id_pog) + '. O izvršenoj otplati objekta leasinga, ' + rtrim(@p_podjetje) + ' izdat će posebno Ovlaštenje s dozvolom prijenosa prava vlasništva na ime kupca.' ,
		...
		begin
				...
				--TODO OVO PODESITI NA SVAKOJ FIRMI POSEBNO InvoiceNote
				InvoiceNote = case when (a.debit < 0 and a.neobdav <= 0) or (a.debit <= 0 and a.neobdav < 0) then '' --c.ddv_id neće nikada biti null case when isnull(c.ddv_id, '') = '' then 'MOLIMO POTVRDITE ISPRAVAK PRETPOREZA POVRATOM OVJERENE KOPIJE STORNA (sukladno čl. 33 stavak 7. Zakona o porezu na dodanu vrijednost) !'  else '' end 
				...
				InvoicePaymentNote = case when (a.debit < 0 and a.neobdav <= 0) or (a.debit <= 0 and a.neobdav < 0) then case when a.old_ddv_d < '2024-01-01' then 'Ispravak promjene porezne osnove temeljem ovjere! (U slučaju da niste primili dokument za ovjeru, molimo da ispravak porezne osnove potvrdite ovjerom ovog računa)' else 'Ovaj dokument je OBAVIJEST da je ' +rtrim(n.p_podjetje) +' proveo ispravak porezne osnovice sukladno članku 43.a Pravilnika o PDV-u koji se primjenjuje na izdane račune od 01.01.2024.' end 
...
begin 
		...
		begin
			update #invoice_data1 set 
				...
				outer apply (
						Select top 1 fa.id_cont, fa.ddv_id, b.val_string
						From najem_fa fa
						inner join (
							Select cast(ke.id_entiteta as int) as id_cont, ke.val_string
							From dbo.kategorije_entiteta ke
							inner join dbo.kategorije_tip b on ke.id_kategorije_tip = b.id_kategorije_tip and b.entiteta = 'POGODBA' and b.neaktiven = 0
							where b.sifra = 'NAZIV_ODJELA') b on fa.id_cont = b.id_cont 
						where isnull(b.val_string, '') != ''
						and fa.ddv_id = a.ddv_id
					) kat_naziv_odjela --on a.ddv_id = kat_naziv_odjela.ddv_id
