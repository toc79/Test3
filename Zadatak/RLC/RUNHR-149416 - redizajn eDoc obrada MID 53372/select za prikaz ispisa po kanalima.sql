...
--06.03.2024 g_tomislav MID 51835 - dorada prikaza rep_ind za FINA partnere kojima je obavijest išla u FAK_LOBR_SSOFT_RLC
--05.04.2024 g_majam MID 52469 - dorada kolone email za EDOC_EX7
...
select * 
	...
	--, ps.REP_NAME as Naziv_opcije_ispisa
	--, res.id_doc_type
	--, res.id_doc_type_lookup
into #main
from (
	...
	where eefc.date_exported >= '20240901'
    ) as SourceTable
pivot 
    (
    sum(exported_to_channel)
    for id_edoc_channel in ([EDOC_EX1], [EDOC_EX2], [EDOC_EX3], [EDOC_EX4], [EDOC_EX5], [EDOC_EX6], [EDOC_EX7], [EDOC_EXPORT_FINA]) --, [EDOC_EX5], [EDOC_EX6] bi se možda mogli isključiti jer za tu statistiku se uzimaju podaci iz xdoc_document
) as PivotTable
--	outer apply 
--		(	select top 1 * 
--			from dbo.print_selection 
--			where REP_KEY = PivotTable.id_report
--		) ps
--left join dbo.reports_edoc_settings res on PivotTable.id_report = res.REPORT_NAME
...
declare @id_xdoc_templateZBIR int = (select id_xdoc_template from dbo.xdoc_template where id_xdoc_key = 'MID52469') -- eDoc to eMail exporter - Zbirni računi   
...
into #xdd_ZBIR
from dbo.xdoc_document
where id_xdoc_template = @id_xdoc_templateZBIR 
and date_exported >= @min_date_prepared
option(recompile) -- inforcess Index seek instead of scan

Select --a.*,
	EDOC_EX1, EDOC_EX2, EDOC_EX3, EDOC_EX4, EDOC_EX5, EDOC_EX6, EDOC_EX7, EDOC_EXPORT_FINA,
    CASE 	-- moglo bi se maknuti id_edoc_doctype za uvjete gdje se gleda/provjerava id_report, Invoice treba detaljnije provjeriti
	    ...
	    WHEN a.id_edoc_doctype = 'Contract' AND LTRIM(RTRIM(a.id_report)) = 'NAROC_SSOFT_RLC' THEN 'Narudžbenica za opremu i vozila'
	...
	, convert(int, case when xdd_ZBIR.doc_id is not null and a.EDOC_EX7 > 0 then 1 else 0 end) as email_ZBIR
...
left join #xdd_ZBIR as xdd_ZBIR on a.id_file = xdd_ZBIR.doc_id

left join dbo.reports_edoc_settings res on a.id_report = res.REPORT_NAME

union all
-- prikaz rep_ind za FINA partnere kojima je obavijest išla u FAK_LOBR_SSOFT_RLC s DDV_ID u napomeni (ne provjerava se za sada FINA ID (ident_stevilka))
select 0 as EDOC_EX1, 0 as EDOC_EX2, 0 as EDOC_EX3, 0 as EDOC_EX4, 0 as EDOC_EX5, 0 as EDOC_EX6, 0 as EDOC_EX7
	, 1 as EDOC_EXPORT_FINA, 'Obavijest o promjeni indeksa' as tip
	, 0 as xw_racun, 0 as email_OBV_IND, 0 as email_LOBR, 0 as email_ZBIR
...
select tip, count(*) as broj
	...
	, sum(email_OBV_IND) + sum(email_LOBR) + sum(email_ZBIR) as email
	, sum(email_OBV_IND) as email_OBV_IND
	, sum(email_LOBR) as email_LOBR
	, sum(email_ZBIR) as email_ZBIR
