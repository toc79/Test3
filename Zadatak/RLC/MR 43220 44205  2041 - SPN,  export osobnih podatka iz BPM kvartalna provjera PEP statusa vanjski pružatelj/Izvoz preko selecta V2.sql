/*===============================
18.09.2019 g_tomislav MR 43220 - created; 
===============================*/
...
SELECT a.id_kupca AS reference, a.naz_kr_kup AS terms 
INTO #tempSNP
FROM dbo.partner a 
...
SELECT DISTINCT customer_id AS reference, related_fo_manage_desc1 AS terms 
FROM (
...
(
	...
	(
		SELECT
			t.reference as '@ID_KUPCA', 
			p.vr_osebe as '@vrsta_osebe',
			'' as  '@Additional_desc'
		FROM #tempSNP t
		INNER JOIN PARTNER p on p.id_kupca=t.reference
		WHERE p.vr_osebe in (SELECT id FROM #tempVrste)
		GROUP BY t.reference, p.vr_osebe
	) as s
