...
--KRAJ DORADE
	...
	, CASE WHEN osig.id_stroska = 'KO' AND ik.id_stroska = 'IK' THEN ' (u izračun EKS uključeni su ' 
		...
		+ CASE WHEN v.id_grupe = 'VBO' THEN 'troškovi kasko osiguranja koji su poznati davatelju leasinga' ELSE 'minimalno zahtjevani troškovi kasko osiguranja, koji su poznati davatelju leasinga, a koji pokrivaju rizik od krađe i totalne štete' END +' u iznosu od '+dbo.gfn_gccif(osig.znesek)+' '+po.id_val+' godišnje'
...
-- LEFT JOIN (SELECT id_pon, id_stroska, SUM(znesek) AS znesek from dbo.pon_terj_stros where id_stroska = 'KO' and id_pon=@Id1 GROUP BY id_pon, id_stroska) osig on po.id_pon=osig.id_pon --dodana suma zato jer će biti uneseni troškovi po godinama/mjesecima
...
Efektivna kamatna stopa iznosi {Format("{0:P2}", Offer.ef_obrm/100)}{Offer.txt_osig}. Iskazana efektivna kamatna stopa vrijedi na datum izrade ponude/otplatne tablice i mijenjati će se ovisno o datumu isporuke objekta leasinga, odnosno datumu aktiviranja leasing ugovora.

1. Izmjene na ponudi u dijelu koji se odnosi na EKS: 
 "Efektivna kamatna stopa iznosi xxx.% (u izračun EKS uključeni su predviđeni minimalno zahtjevani troškovi kasko osiguranja, koji su poznati davatelju leasinga, a koji pokrivaju rizik od krađe i totalne štete u iznosu od xxx EUR godišnje i interkalarna kamata za period od 30 dana)." 

Ovaj gore navedeni tekst trebao bi se ispisivati na sljedećoj grupi opreme: VNC (nova vozila), VUC (rabljena vozila) i VLT (laka komercijalna). 

2. za grupu opreme VBO (plovila) zatražiti da se ispisuje na ponudi drugačiji tekst jer će se raditi o punoj kasko polici 

Tekst ponude za plovila bi glasio: 
"Efektivna kamatna stopa iznosi xxx.% (u izračun EKS uključeni su predviđeni minimalno zahtjevani troškovi kasko osiguranja koji su poznati davatelju leasinga koji pokrivaju rizik od krađe i totalne štete u iznosu od xxx EUR godišnje i interkalarna kamata za period od 30 dana)." 

3.  za rabljene objekte leasinga dodati kod izrade ponude u polju Troškovi novu kategoriju za unos i izračun EKS-a pod nazivom "Procjena rabljenog objekta leasinga" - ukoliko postoji dodatni trošak procjene ispis ponude treba glasiti, na način da se u tekst doda infomacija o trošku procjene - dolje označeno plavo: 

 "Efektivna kamatna stopa iznosi xxx.% (u izračun EKS uključeni su predviđeni minimalno zahtjevani troškovi kasko osiguranja koji su poznati davatelju leasinga, a koji pokrivaju rizik od krađe i totalne štete u iznosu od xxx EUR godišnje, trošak procjene objekta leasinga u iznosu od xxx EUR jednokratno i interkalarna kamata za period od 30 dana)." 
