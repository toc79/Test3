INSERT INTO dbo.vrst_ter_fikt(id_stroska,sifra,opis,rac_eom,davek) VALUES('PR','PROL','Procjena rabljenog objekta leasinga',1,NULL)
...
select @add = case when @nacin_leas = ''F1'' and isnull(@je_fo,0) = 1 then 1 else 0 end','','','','PR',NULL,NULL,0,0,14,1,0)
--F2
INSERT INTO dbo.kalk_form_stros(pogoj,str_of,df_str,df_str_p,id_stroska,id_terj,opis,disable_str,disable_str_p,vrstni_red,predlaga_na_ponudbi,neaktiven) VALUES('declare @id_kupca varchar(6), @nacin_leas char(2), @fochar varchar(4)
set @id_kupca = ''{kup}''
set @nacin_leas = ''{nl}''
...
select @add = case when @nacin_leas = ''F2'' and isnull(@je_fo,0) = 1 then 1 else 0 end','','','','PR',NULL,NULL,0,0,14,1,0)
...
select @add = case when @nacin_leas = ''F3'' and isnull(@je_fo,0) = 1 then 1 else 0 end','','','','PR',NULL,NULL,0,0,14,1,0)
...
select @add = case when @nacin_leas = ''F4'' and isnull(@je_fo,0) = 1 then 1 else 0 end','','','','PR',NULL,NULL,0,0,14,1,0)
select * from dbo.kalk_form_stros where neaktiven = 0