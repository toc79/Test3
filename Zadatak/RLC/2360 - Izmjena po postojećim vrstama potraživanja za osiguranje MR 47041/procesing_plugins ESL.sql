...
begin
	...
	CASE WHEN (b.source in ('NAJEM_FA', 'ZOBR_FA', 'AVANSI', 'TEC_RAZL') OR (b.source = 'GL_OUTPUT_R' and rac_out.id_dav_st = 'AV') or (b.source = 'ZA_OPOM' and o.st_opomina != 3)) and @is_for_fina = 0 THEN 1 
...
if @DocType = 'General' AND @ReportName = 'OBV_OPC_SSOFT_ESL'
begin 
	Select
...
if @DocType = 'General' AND @ReportName = 'OBVREGT_SSOFT_ESL'
begin 
	Select
	'OBAVIJEST O ISTEKU REGISTRACIJE I POLICA OSIGURANJA ' + RTRIM(p.id_pog) as [gmi.earchive.doc_title], 
	'OBAVIJEST O ISTEKU REGISTRACIJE I POLICA OSIGURANJA ' + RTRIM(p.id_pog) as [gmi.earchive.doc_description],
    RTRIM(par.id_kupca) + '_HP_' + RTRIM(@DocType) + '_' + RTRIM(@Id) + '_' + rtrim(isnull(b.rendered_by, '')) + '.pdf' As print_centar_name, 
	1 as [edoc.for_print_centar],
	1 as [edoc.export_earchive],
	'DOPIS ZA REGISTRACIJU' as [tip_dokumenta],
	RTRIM(par.dav_stev) as oib,
	p.id_cont, RTRIM(p.id_pog) as id_pog, RTRIM(par.id_kupca) as id_kupca, RTRIM(par.naz_kr_kup) as naz_kr_kup,
	b.rendered_when as datum_ispisa
	From dbo.za_regis a
	...
	INNER JOIN dbo.partner par on p.id_kupca = par.id_kupca
	INNER JOIN dbo.reports_log b on CAST(a.id_za_regis as varchar(100)) = @id AND b.doc_type = 'General' AND b.edoc_file_name = @OriginalFileName
	--left join dbo.poste pos on par.id_poste = pos.id_poste 
	WHERE b.id_object_edoc = @id AND @DocType = 'General'
end

if @DocType = 'General' AND @ReportName = 'PZ_DOPIS_SSOFT_ESL'
begin 
	Select
	...
	INNER JOIN dbo.pogodba p on a.id_cont = p.id_cont
	INNER JOIN dbo.partner par on p.id_kupca = par.id_kupca
	INNER JOIN dbo.reports_log b on CAST(a.id_za_pz as varchar(100)) = @id AND b.doc_type = 'General' AND b.edoc_file_name = @OriginalFileName
	--left join dbo.poste pos on par.id_poste = pos.id_poste 
	WHERE b.id_object_edoc = @id AND @DocType = 'General'
end
...
begin 
	...
	0 as [edoc.export_earchive],
	RTRIM(par.dav_stev) as oib
	from dbo.gv_za_opom_with_arh a
	Inner join dbo.gv_PogodbaAll b on a.id_cont = b.id_cont
	Inner Join dbo.pog_poro por ON a.id_cont = por.id_cont and por.id_poroka = cast(substring(@id, charindex('$',@id)+1, len(rtrim(@id))) as char(6))
...
Begin
	...
	begin 
		...
		declare @referencesXml varchar(max), @ubl_tip varchar(5), @xml as xml, @addPropertyXml varchar(max) 
	
		set @source = dbo.gfn_GetInvoiceSource(@Id)
		Select @p_zrac = cast(rtrim(p_zrac) as varchar(50)), @p_podjetje = cast(rtrim(p_podjetje) as varchar(100)) From dbo.nastavit
		set @addCostXml = ''
		set @invoiceLineXml = ''
		set @referencesXml = ''
		set @taxTotalXml = ''
