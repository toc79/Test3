...
if @DocType = 'Contract' and @ReportName = 'PLANP_SSOFT_BKS'
begin
	Select 
	'Plan otplate ' + RTRIM(p.id_pog) + ' ' + RTRIM(@Id) as [gmi.earchive.doc_title], 
	'Plan otplate ' + RTRIM(p.id_pog) + ' ' + RTRIM(@Id) as [gmi.earchive.doc_description], 
	...
	where cast(p.id_cont as varchar(100)) =  @Id and @DocType = 'Contract' and @ReportName = 'PLANP_SSOFT_BKS'
end

if @DocType = 'Contract' and @ReportName = 'PREDRPOL_SSOFT_BKS'
begin
	Select 
	'Poziv na plaćanje' + RTRIM(p.id_pog) + ' ' + RTRIM(@Id) as [gmi.earchive.doc_title], 
	'Poziv na plaćanje' + RTRIM(p.id_pog) + ' ' + RTRIM(@Id) as [gmi.earchive.doc_description], 
	'POZIV NA PLAĆANJE' as [tip_dokumenta],
	par.naz_kr_kup as [partner_title],
	RTRIM(par.dav_stev) as [oib],
	CONVERT(date, getdate()) as [document_date],
	'1' as [edoc.for_nova_arhiva]
	FROM dbo.gv_PogodbaAll p 
	LEFT JOIN dbo.partner par ON p.id_kupca = par.id_kupca
	where cast(p.id_cont as varchar(100)) =  @Id and @DocType = 'Contract' and @ReportName = 'PREDRPOL_SSOFT_BKS'
end
...
--g_majam MR51214
if @DocType = 'Contract' and @ReportName = 'POZIV_DOK_SSOFT_BKS'
begin
	Select 
	'Poziv na dostavu dokumentacije ' + RTRIM(a.id_pog) + ' ' + RTRIM(@Id) as [gmi.earchive.doc_title], 
	'Poziv na dostavu dokumentacije ' + RTRIM(a.id_pog) + ' ' + RTRIM(@Id) as [gmi.earchive.doc_description], 
	'POZIV NA DOSTAVU DOKUMENTACIJE' as [tip_dokumenta],
	a.id_cont as [contract_id],
	RTRIM(a.id_pog) as [contract_number],
	RTRIM(par.id_kupca) as [partner_id],
	par.naz_kr_kup as [partner_title],
	RTRIM(par.dav_stev) as [oib],
	CONVERT(date, getdate()) as [document_date],
	'1' as [edoc.for_nova_arhiva]
...
--g_igorp MR51942
IF @DocType = 'General' and @ReportName = 'KON_OBR_SSOFT_BKS'
begin
	Select
	'Konačni obračun po ugovoru ' + RTRIM(a.id_pog) as [gmi.earchive.doc_title],
	'Konačni obračun po ugovoru ' + RTRIM(a.id_pog) as [gmi.earchive.doc_description],
	...
	FROM dbo.pogodba a
	LEFT JOIN dbo.partner par ON A.id_kupca = par.id_kupca
	where cast(a.id_cont as varchar(100)) = @Id and @DocType = 'General'
END

IF @DocType = 'General' and @ReportName = 'IZJ_O_PL_POT_SSOFT_BKS'
begin
	Select
	'Izjava o plaćanju svih potraživanja po ugovoru ' + RTRIM(a.id_pog) as [gmi.earchive.doc_title],
	'Izjava o plaćanju svih potraživanja po ugovoru ' + RTRIM(a.id_pog) as [gmi.earchive.doc_description],
	'IZJAVA O PLAĆANJU SVIH PTORAŽIVANJA' as [tip_dokumenta],
	a.id_cont as [contract_id],
	RTRIM(a.id_pog) as [contract_number],
	RTRIM(a.id_kupca) as [partner_id],
	RTRIM(par.naz_kr_kup) as [partner_title],
