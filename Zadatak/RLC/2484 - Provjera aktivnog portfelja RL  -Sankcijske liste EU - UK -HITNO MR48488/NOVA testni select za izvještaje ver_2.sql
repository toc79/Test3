Provjeriti duljinu provjere za sve parntere jer nova funkcija sada radi brže!?

-- 14.03.2022 g_tomislav MID 48488 - created. Final check is based by gfn_CheckInternationalBlackListSummary
-- promjeniti npr. naziv1_kup u Naziv 1 partnera kako je na mapi partnera
select par.id_kupca, cast(naziv1_kup as varchar(150)) as naziv1_kup
...
from (
	select id_kupca, naziv1_kup, direktor, kontakt, delodajale, kontakt_d
		, ime, priimek, vr_osebe
		, drzavljan, drzavljanstvo, id_poste_sed, id_poste, id_poste_d, id_poste_k
	from #all_partners
) a
unpivot 
	(nazivPartnera for polje in ([naziv1_kup], [direktor], [kontakt], [delodajale], [kontakt_d])
) b
