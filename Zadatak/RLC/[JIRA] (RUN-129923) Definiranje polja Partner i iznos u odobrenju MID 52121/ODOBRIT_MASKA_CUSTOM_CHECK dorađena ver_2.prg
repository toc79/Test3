...
IF RECCOUNT("_ef_B1_B6") > 0
	...
	GF_SQLEXEC("select opcija, ((ost_obr_n * traj_naj) + opcija) * 1.1 as iznosZaB3_B6, id_tec from dbo.ponudba where id_pon = " +GF_Quotedstr(ALLT(_odobrit.id_pon)), "_ef_iznosiZaB1_B6")
	
	lcPoruka3 = ""
	
	SELE _ef_B1_B6
	SCAN
		IF INLIST(_ef_B1_B6.id_obl_zav, "B1", "B2") AND _ef_B1_B6.id_vr_val != GF_XCHANGE(_ef_B1_B6.id_tec, _ef_iznosiZaB1_B6.opcija, _ef_iznosiZaB1_B6.id_tec, DATE())
			lcPoruka3 = lcPoruka3 + "Dokumentu " +_ef_B1_B6.id_obl_zav +" za iznos mora biti unesen Ostatak vrijednosti koji iznosi " +TRANS(_ef_iznosiZaB1_B6.opcija, GCCIF) +GCE
		ENDIF
		IF INLIST(_ef_B1_B6.id_obl_zav, "B3", "B4", "B5", "B6") AND _ef_B1_B6.id_vr_val != GF_XCHANGE(_ef_B1_B6.id_tec, _ef_iznosiZaB1_B6.iznosZaB3_B6, _ef_iznosiZaB1_B6.id_tec, DATE())
			lcPoruka3 = lcPoruka3 + "Dokumentu " +_ef_B1_B6.id_obl_zav +" za iznos mora biti unesen " +TRANS(_ef_iznosiZaB1_B6.iznosZaB3_B6, GCCIF) +" (po formuli rata * broj mj. + ost. vrijednosti + 10% ukupnog iznosa)" +GCE
		ENDIF
	ENDSCAN
	
	IF !EMPTY(lcPoruka3) 
		*IF RF_MSGBOX("Pozor", lcPoruka3 + "Nastavak spremanja nije dozvoljen." +GCE +"Da li želite ručno upisati iznose ili da se iznosi automatski promjene?", "Ručno", "Automatski", "Poništi") 
		*IF POTRJENO("Pozor", lcPoruka3 + "Nastavak spremanja nije dozvoljen." +GCE +"Da li želite ručno upisati iznose ili da se iznosi automatski promjene?", "Ručno", "Automatski", "Poništi") 
		POZOR("Pozor", lcPoruka3 + "Nastavak spremanja nije dozvoljen bez unosa navedenih iznosa.") 
		RETURN .F.
		* DO CASE
		* CASE lnOdg = 2 && Automatski
