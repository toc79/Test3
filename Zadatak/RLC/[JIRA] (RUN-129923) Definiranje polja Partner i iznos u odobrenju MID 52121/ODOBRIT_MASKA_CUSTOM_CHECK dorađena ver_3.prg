...
IF RECCOUNT("_ef_B1_B6") > 0
	...
	SCAN
		...
		IF INLIST(_ef_B1_B6.id_obl_zav, "B1", "B2") AND lnId_vr_val != GF_XCHANGE(lcId_tec, _ef_iznosiZaB1_B6.opcija, _ef_iznosiZaB1_B6.id_tec, DATE())
			lcPoruka3 = lcPoruka3 + "Dokumentu " +_ef_B1_B6.id_obl_zav +" za iznos mora biti unesen Ostatak vrijednosti koji iznosi " +ALLT(TRANS(_ef_iznosiZaB1_B6.opcija, GCCIF)) +"." +GCE
		ENDIF
		IF INLIST(_ef_B1_B6.id_obl_zav, "B3", "B4", "B5", "B6") AND lnId_vr_val != GF_XCHANGE(lcId_tec, _ef_iznosiZaB1_B6.iznosZaB3_B6, _ef_iznosiZaB1_B6.id_tec, DATE())
			lcPoruka3 = lcPoruka3 + "Dokumentu " +_ef_B1_B6.id_obl_zav +" za iznos mora biti unesen " +ALLT(TRANS(_ef_iznosiZaB1_B6.iznosZaB3_B6, GCCIF)) +" (po formuli rata * broj mj. + ost. vrijednosti + 10% ukupnog iznosa)" +"." +GCE
		ENDIF
	ENDSCAN
	
	IF !EMPTY(lcPoruka3) 
		POZOR(lcPoruka3 + "Nastavak spremanja nije dozvoljen bez unosa navedenih iznosa.") 
		RETURN .F.
	ENDIF
ENDIF
** END MID 52121 *****
