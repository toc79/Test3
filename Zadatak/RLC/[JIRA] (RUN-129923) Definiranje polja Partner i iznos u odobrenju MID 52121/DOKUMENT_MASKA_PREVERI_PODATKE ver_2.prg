...
***************************************************
** 15.10.2024. g_tomislav MID 51121 

ldZacetek = dokument.zacetek

IF dokument.id_obl_zav == "ZT" AND !GF_NULLOREMPTY(ldZacetek)
	ldVelja_do = NVL(dokument.velja_do, {01.01.1900 00:00:00})
	lnPomak = 10
	ldVelja_doNew = DATE(YEAR(ldZacetek) +lnPomak, MONTH(ldZacetek), DAY(ldZacetek))   &&TODO LEAP YEAR 
	
	IF TTOD(ldVelja_do) != ldVelja_doNew AND POTRJENO("Datum 'Vrijedi do' mora biti " +ALLT(TRANS(ldVelja_doNew)) +" (+" +ALLT(STR(lnPomak)) +" godina od datuma 'Početak')!" +GCE +"Da li želite promijeniti datum 'Vrijedi do' na " +ALLT(TRANS(ldVelja_doNew)) +"?")
		loForm.txtVelja_do.Value = ldVelja_doNew
	ENDIF
ENDIF
** KRAJ MID 51690***********************************