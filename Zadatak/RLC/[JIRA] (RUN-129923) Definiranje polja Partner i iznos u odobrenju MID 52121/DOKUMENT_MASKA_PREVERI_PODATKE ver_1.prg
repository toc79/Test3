...
IF dokument.id_obl_zav == "ZT" AND !GF_NULLOREMPTY(ldZacetek)
	...
	lnPomak = 10
	IF !(DAY(ldZacetek) == DAY(ldVelja_do) AND MONTH(ldZacetek) == MONTH(ldVelja_do) AND YEAR(ldZacetek) +lnPomak == YEAR(ldVelja_do)) ;
	AND !POTRJENO("Datum 'Vrijedi do' mora bi unesen na +" +ALLT(STR(lnPomak)) +" godina od datuma 'Početak'!" +GCE +"Da li želite nastaviti sa spremanjem?")
		RETURN .F.
	ENDIF
ENDIF
** KRAJ MID 51690***********************************