...
-- 20.05.2015 Andrej; Bug 31702 - modified columns datatype for tables @result and @frame_candidates. Columns: opis, kraj, opombe, tecaj, obr_mera, int_obr_mera, konto, pkonto, pkonto_davek, int_opis_fak, limproc_pol, limobr_mera, limtraj_naj, opis_zav, l
improc_varsc, limproc_opcija
...
------------------------------------------------------------------------------------------------------------
create               FUNCTION [dbo].[gfn_FrameView](
...
RETURN(
  ...
  SELECT F.id_parent, F.je_krovni_okvir, 
        ...
        vr_val = ISNULL(CASE FT.sif_frame_type
             ...
             WHEN 'REV' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O.obligo_rev, CASE WHEN P.status_akt = 'Z' OR (P.status_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P.vr_val - P.varscina END), ISNULL(O.id_tec, P.id_tec), 
@par_trenutnidatum))
			 ...
			 WHEN 'RFO' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O.obligo_rfo, CASE WHEN P.status_akt = 'Z' OR (P.status_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE dbo.gfn_VrValToBrutoInternal(p.vr_val, p.robresti_val, ds.davek, 
nl.ima_robresti, nl.dav_b, nl.finbruto, nl.dav_n) + P.man_str + P.stroski_x + P.stroski_pz + P.stroski_zt + P.zav_fin + P.str_financ END), ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
			 WHEN 'RRE' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE V.znesek_dom END, '000', @par_trenutnidatum))
			 WHEN 'RNE' THEN SUM(dbo.gfn_XChange(F.id_tec,  CASE WHEN nl.ddv_takoj = 1 THEN P.net_nal_zac ELSE 0 END,P.id_tec, P.dat_sklen)) + SUM(dbo.gfn_XChange(F.id_tec,  CASE WHEN nl.ddv_takoj = 1 THEN (ISNULL(O.obligo_rne, CASE WHEN P.status_akt = 'Z' OR (P.s
tatus_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P.DDV END)) ELSE 0 END, ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
...
tatus_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P.DDV END)) ELSE 0 END, ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
        ...
        vr_dom = ISNULL(CASE FT.sif_frame_type 
             ...
             WHEN 'REV' THEN SUM(dbo.gfn_XChange('000', ISNULL(O.obligo_rev, CASE WHEN P.status_akt = 'Z' OR (P.status_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P.vr_val - P.varscina END), ISNULL(O.id_tec, P.id_tec), @pa
r_trenutnidatum))
			 ...
			 WHEN 'RFO' THEN SUM(dbo.gfn_XChange('000', ISNULL(O.obligo_rfo, CASE WHEN P.status_akt = 'Z' OR (P.status_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE dbo.gfn_VrValToBrutoInternal(p.vr_val, p.robresti_val, ds.davek, nl.
ima_robresti, nl.dav_b, nl.finbruto, nl.dav_n) + P.man_str + P.stroski_x + P.stroski_pz + P.stroski_zt + P.zav_fin + P.str_financ END), ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
			 WHEN 'RRE' THEN SUM(CASE WHEN F.status_akt = 'Z' THEN 0 ELSE V.znesek_dom END)
			 WHEN 'RNE' THEN SUM(dbo.gfn_XChange('000',  CASE WHEN nl.ddv_takoj = 1 THEN P.net_nal_zac ELSE 0 END,P.id_tec, P.dat_sklen)) + SUM(dbo.gfn_XChange('000',  CASE WHEN nl.ddv_takoj = 1 THEN (ISNULL(O.obligo_rne, CASE WHEN P.status_akt = 'Z' OR (P.status_
akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P.DDV END)) ELSE 0 END, ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
    ...
    FROM 
        ...
        LEFT JOIN (SELECT pp.id_cont, pp.id_kupca, pp.id_tec,
							SUM(case when upper(IsNull(cs.val, '')) = 'TRUE'
										then pp.znp_saldo_brut_all + pp.poknj_nezap_debit_brut_ALL + pp.bod_neto_lpod - pp.poknj_nezap_neto_lpod + pp.bod_robresti_lpod - pp.poknj_nezap_robresti_lpod + case when nl.leas_kred = 'L' and nl.tip_knjizenja = '2' then pp.bod_davek_LPOD - pp.
poknj_nezap_davek_LPOD else 0 end
    ...
    HAVING (@par_razlika = 0 OR (F.znesek_val - ISNULL(CASE FT.sif_frame_type
                                                         WHEN 'POG' THEN SUM(dbo.gfn_XChange(F.id_tec, P.vr_val_zac, P.id_tec, P.dat_sklen))
                                                         WHEN 'PLA' THEN SUM(dbo.gfn_XChange(F.id_tec, FP.znesek_pl, '000', FP.datum_pl) - dbo.gfn_XChange(F.id_tec, FP.odbitni_ddv, '000', FP.datum_pl))
                                                         WHEN 'REV' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O.obligo_rev, CASE WHEN P.status_akt = 'Z' OR (P.status_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P.vr_val - P
.varscina END), ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
                                                         WHEN 'NET' THEN SUM(dbo.gfn_XChange(F.id_tec, P.net_nal, P.id_tec, P.dat_sklen))
                                                         WHEN 'DOB' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE R.znesek_dom END, '000', @par_trenutnidatum))
														 WHEN 'ZAL' THEN SUM(STK.kredit - STK.debit)
														 WHEN 'RFO' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O.obligo_rfo, CASE WHEN P.status_akt = 'Z' OR (P.status_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE dbo.gfn_VrValToBrutoInternal(p.vr_val, p.robresti_val,
 ds.davek, nl.ima_robresti, nl.dav_b, nl.finbruto, nl.dav_n) + P.man_str + P.stroski_x + P.stroski_pz + P.stroski_zt + P.zav_fin + P.str_financ END), ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
														 WHEN 'RRE' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE V.znesek_dom END, '000', @par_trenutnidatum))
														 WHEN 'RNE' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN nl.ddv_takoj = 1 THEN P.net_nal_zac ELSE 0 END,P.id_tec, P.dat_sklen)) + SUM(dbo.gfn_XChange(F.id_tec,  CASE WHEN nl.ddv_takoj = 1 THEN (ISNULL(O.obligo_rne, CASE WHEN P.status_akt = '
Z' OR (P.status_akt = 'A' AND DATEDIFF(dd, P.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P.DDV END)) ELSE 0 END, ISNULL(O.id_tec, P.id_tec), @par_trenutnidatum))
    ...
    SELECT
        ...
        vr_val = ISNULL(CASE FT.sif_frame_type
             ...
             WHEN 'PLA' THEN SUM(dbo.gfn_XChange(F.id_tec, FP.znesek_pl, '000', FP.datum_pl) - dbo.gfn_XChange(F.id_tec, FP.odbitni_ddv, '000', FP.datum_pl))
             WHEN 'REV' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O1.obligo_rev, CASE WHEN P1.status_akt = 'Z' OR (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P1.vr_val - P1.varscina END), ISNULL(O1.id_tec, P1.i
d_tec), @par_trenutnidatum))
             WHEN 'NET' THEN SUM(dbo.gfn_XChange(F.id_tec, P1.net_nal, P1.id_tec, P1.dat_sklen))
             WHEN 'DOB' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE R.znesek_dom END, '000', @par_trenutnidatum))
             WHEN 'ZAL' THEN SUM(dbo.gfn_XChange(F.id_tec, STK.kredit - STK.debit, '000', @par_trenutnidatum))
			 WHEN 'RFO' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O1.obligo_rfo, CASE WHEN P1.status_akt = 'Z' OR (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE dbo.gfn_VrValToBrutoInternal(p1.vr_val, p1.robresti_val, ds.d
avek, nl.ima_robresti, nl.dav_b, nl.finbruto, nl.dav_n) + P1.man_str + P1.stroski_x + P1.stroski_pz + P1.stroski_zt + P1.zav_fin + P1.str_financ END), ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
			 WHEN 'RRE' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE V.znesek_dom END, '000', @par_trenutnidatum))
			 WHEN 'RNE' THEN SUM(dbo.gfn_XChange(F.id_tec,  CASE WHEN nl.ddv_takoj = 1 THEN P1.net_nal_zac ELSE 0 END,P1.id_tec, P1.dat_sklen)) + SUM(dbo.gfn_XChange(F.id_tec,  CASE WHEN nl.ddv_takoj = 1 THEN (ISNULL(O1.obligo_rne, CASE WHEN P1.status_akt = 'Z' OR
 (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P1.DDV END)) ELSE 0 END, ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
			 WHEN 'DBA' THEN SUM(dbo.gfn_XChange(F.id_tec, S.znesek_dom, '000', @par_trenutnidatum))
           END, 0),
        razlika_val = F.znesek_val - ISNULL(CASE FT.sif_frame_type
             WHEN 'POG' THEN SUM(dbo.gfn_XChange(F.id_tec, P1.vr_val_zac, P1.id_tec, P1.dat_sklen))
...
avek, nl.ima_robresti, nl.dav_b, nl.finbruto, nl.dav_n) + P1.man_str + P1.stroski_x + P1.stroski_pz + P1.stroski_zt + P1.zav_fin + P1.str_financ END), ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
 ...
 (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P1.DDV END)) ELSE 0 END, ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
        ...
        vr_dom = ISNULL(CASE FT.sif_frame_type 
             ...
             WHEN 'PLA' THEN SUM(FP.znesek_pl - FP.odbitni_ddv)
             WHEN 'REV' THEN SUM(dbo.gfn_XChange('000', ISNULL(O1.obligo_rev, CASE WHEN P1.status_akt = 'Z' OR (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P1.vr_val - P1.varscina END), ISNULL(O1.id_tec, P1.id_t
ec), @par_trenutnidatum))
             WHEN 'NET' THEN SUM(dbo.gfn_XChange('000', P1.net_nal, P1.id_tec, P1.dat_sklen))
             WHEN 'DOB' THEN SUM(CASE WHEN F.status_akt = 'Z' THEN 0 ELSE R.znesek_dom END)
             WHEN 'ZAL' THEN SUM(STK.kredit - STK.debit)
			 WHEN 'RFO' THEN SUM(dbo.gfn_XChange('000', ISNULL(O1.obligo_rfo, CASE WHEN P1.status_akt = 'Z' OR (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE dbo.gfn_VrValToBrutoInternal(p1.vr_val, p1.robresti_val, ds.dave
k, nl.ima_robresti, nl.dav_b, nl.finbruto, nl.dav_n) + P1.man_str + P1.stroski_x + P1.stroski_pz + P1.stroski_zt + P1.zav_fin + P1.str_financ END), ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
			 WHEN 'RRE' THEN SUM(CASE WHEN F.status_akt = 'Z' THEN 0 ELSE V.znesek_dom END)
			 WHEN 'RNE' THEN SUM(dbo.gfn_XChange('000',  CASE WHEN nl.ddv_takoj = 1 THEN P1.net_nal_zac ELSE 0 END,P1.id_tec, P1.dat_sklen)) + SUM(dbo.gfn_XChange('000',  CASE WHEN nl.ddv_takoj = 1 THEN (ISNULL(O1.obligo_rne, CASE WHEN P1.status_akt = 'Z' OR (P1.s
tatus_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P1.DDV END)) ELSE 0 END, ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
			 WHEN 'DBA' THEN SUM(S.znesek_dom)
           END, 0),
        razlika_dom = F.znesek_dom - ISNULL(CASE FT.sif_frame_type 
             WHEN 'POG' THEN SUM(dbo.gfn_XChange('000', P1.vr_val_zac, P1.id_tec, P1.dat_sklen))
...
poknj_nezap_davek_LPOD else 0 end
    ...
    HAVING (@par_razlika = 0 OR (F.znesek_val - ISNULL(CASE FT.sif_frame_type
                                                         WHEN 'POG' THEN SUM(dbo.gfn_XChange(F.id_tec, P1.vr_val_zac, P1.id_tec, P1.dat_sklen))
                                                         WHEN 'PLA' THEN SUM(dbo.gfn_XChange(F.id_tec, FP.znesek_pl, '000', FP.datum_pl) - dbo.gfn_XChange(F.id_tec, FP.odbitni_ddv, '000', FP.datum_pl))
                                                         WHEN 'REV' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O1.obligo_rev, CASE WHEN P1.status_akt = 'Z' OR (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P1.vr_va
l - P1.varscina END), ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
                                                         WHEN 'NET' THEN SUM(dbo.gfn_XChange(F.id_tec, P1.net_nal, P1.id_tec, P1.dat_sklen))
                                                         WHEN 'DOB' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE R.znesek_dom END, '000', @par_trenutnidatum))
														 WHEN 'ZAL' THEN SUM(STK.kredit - STK.debit)
														 WHEN 'RFO' THEN SUM(dbo.gfn_XChange(F.id_tec, ISNULL(O1.obligo_rfo, CASE WHEN P1.status_akt = 'Z' OR (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE dbo.gfn_VrValToBrutoInternal(p1.vr_val, p1.robrest
i_val, ds.davek, nl.ima_robresti, nl.dav_b, nl.finbruto, nl.dav_n) + P1.man_str + P1.stroski_x + P1.stroski_pz + P1.stroski_zt + P1.zav_fin + P1.str_financ END), ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
														 WHEN 'RRE' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE V.znesek_dom END, '000', @par_trenutnidatum))
														 WHEN 'RNE' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN nl.ddv_takoj = 1 THEN P1.net_nal_zac ELSE 0 END,P1.id_tec, P1.dat_sklen)) + SUM(dbo.gfn_XChange(F.id_tec,  CASE WHEN nl.ddv_takoj = 1 THEN (ISNULL(O1.obligo_rne, CASE WHEN P1.status_ak
t = 'Z' OR (P1.status_akt = 'A' AND DATEDIFF(dd, P1.dat_aktiv, @par_trenutnidatum) > 5) THEN 0 ELSE P1.DDV END)) ELSE 0 END, ISNULL(O1.id_tec, P1.id_tec), @par_trenutnidatum))
														 WHEN 'DBA' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE R.znesek_dom END, '000', @par_trenutnidatum))
                                                       END, 0)) < 0)
)