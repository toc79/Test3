USE [NOVA_TEST]
GO
/****** Object:  UserDefinedFunction [dbo].[gfn_FrameView]    Script Date: 8.4.2016. 11:05:08 ******/
SET ANSI_NULLS ON
GO
...
-- 01.03.2016 Andrej; MID 55837 - added custom setting check 'gfn_FrameView_DATE_TYPE' for calculating vr_val and razlika_val for sif_frame_type DOB and DBA 
...
RETURN(
  ...
  SELECT F.id_parent, F.je_krovni_okvir, 
        ...
        vr_val = ISNULL(CASE FT.sif_frame_type
             ...
             WHEN 'DOB' THEN SUM(dbo.gfn_XChange(F.id_tec, CASE WHEN F.status_akt = 'Z' THEN 0 ELSE R.znesek_dom END, '000', CASE WHEN dbo.gfn_GetCustomSettings('gfn_FrameView_DATE_TYPE') = 'datum_pl' THEN FP.datum_pl ELSE @par_trenutnidatum END))
			 ...
			 WHEN 'DBA' THEN SUM(dbo.gfn_XChange(F.id_tec, S.znesek_dom, '000', CASE WHEN dbo.gfn_GetCustomSettings('gfn_FrameView_DATE_TYPE') = 'datum_pl' THEN FP.datum_pl ELSE @par_trenutnidatum END))
           END, 0),
