...
-- 18.05.2022 g_tomislav MID 47763 - prikaz razgraničenja automatski generirana i ona ručno unesena (veza s iznosom je dodana jer imaju slučaj dvije stavke s istim brojem ugovora i kontom)
...
from (
	...
	inner join ( -- lista razgraničenja i šifre troška
		...
		inner join dbo.ARH_GL_INPUT_RK irk on ir.id_gl_input_r = irk.id_gl_input_r and a.id_cont = irk.id_cont and a.raz_pkonto = irk.protikonto and a.znesek = irk.znesek -- veza broj ugovora i konto razgraničenja i iznos
		where a.source_tbl != 'gl_input_rk'
		and a.vrsta_dok = 'PFA' 
		and irk.id_vrst_dod_str is not null
	) r on a.id_gl_razmej = r.id_gl_razmej
	...
	inner join dbo.vrst_dod_str vds on r.id_vrst_dod_str = vds.id_vrst_dod_str
