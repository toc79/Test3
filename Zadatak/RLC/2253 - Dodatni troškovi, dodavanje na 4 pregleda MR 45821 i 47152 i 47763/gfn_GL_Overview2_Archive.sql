...
-- 08.11.2007 Vik; Bug id 26924 - created as separate function that accesses only archive table  
...
-- 10.09.2009 PetraR; Bug ID 27973 - added condition for year for protikonto akonplan_arhiv  
...
-- 22.04.2011 MatjazB; MID 29750 - change LEFT to INNER join for akonplan_deleted for konto and added field year to join (insead of in WHERE clause)  
-- 04.09.2013 Jelena; TID 7509 - added field id_project, projectnumber and projectname  
-- 21.03.2018 Nejc; TID 12991 - GDPR  
------------------------------------------------------------------------------------------------------------  
  
CREATE FUNCTION [dbo].[gfn_GL_Overview2_Archive] (  
 @par_datum_datum int,  
 @id_kupca varchar(6),  
...
RETURN (  
 SELECT   
  ...
  INNER JOIN dbo.akonplan_arhiv A ON G.konto = A.konto AND g.year = a.year  
  LEFT JOIN dbo.sif_skis H ON C.id_skis = H.id_skis  
  LEFT JOIN dbo.akonplan_arhiv AP ON G.protikonto = AP.konto AND g.year = ap.year  
  LEFT JOIN dbo.users U ON G.vnesel = U.username  
  LEFT JOIN dbo.projects P on P.id_project = G.id_project  
 WHERE   
  g.year = @par_datum_datum  
)  
