...
select * into #temp471521 -- ovo izbaciti u select više
from (
...
) r where 1 = (CASE WHEN @par_status_akt_enabled = 1 THEN (CASE WHEN @par_status_akt_type = 1 THEN (CASE WHEN CHARINDEX(r.status,@par_status_akt_value) = 0 THEN 1 ELSE 0 END) ELSE (CASE WHEN CHARINDEX(r.status,@par_status_akt_value) = 0 THEN 0 ELSE 1 END) END) ELSE 1 END)
