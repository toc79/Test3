...
-- 30.09.2021 g_andrijap MID 47640 - dodana kriterij status razgraničenja
-- 09.05.2022 g_tomislav MID 47763 - promjena logike te se sada umjesto a.source_tbl = 'gl_input_rk' prikazuju sva PFA razgraničenja. Umjesto veze a.id_source = irk.ID_GL_INPUT_RK je dodana nova ir.id_gl_input_r = irk.id_gl_input_r and a.id_cont = irk.id_cont and a.raz_pkonto = irk.protikonto, naime sve stavke izlaznog računa koji imaju id_vrst_dod_str imaju i id_cont, a veza s protikonto je dodana jer imaju slučaj dvije stavke s istim brojem ugovora a različitim kontom
...
declare @konto char(8) = {6}
declare @par_status_akt_enabled as bit = {7}
declare @par_status_akt_type as int = {8}
declare @par_status_akt_value as char(1) ={9}
...
from (
	...
	inner join dbo.ARH_GL_INPUT_RK irk on ir.id_gl_input_r = irk.id_gl_input_r and a.id_cont = irk.id_cont and a.raz_pkonto = irk.protikonto -- veza broj ugovora i konto razgraničenja --a.id_source = irk.ID_GL_INPUT_RK
	inner join dbo.vrst_dod_str vds on irk.id_vrst_dod_str = vds.id_vrst_dod_str
	where a.vrsta_dok = 'PFA' --a.source_tbl = 'gl_input_rk'
...
where 1 = (CASE WHEN @par_status_akt_enabled = 1 THEN (CASE WHEN @par_status_akt_type = 1 THEN (CASE WHEN CHARINDEX(r.status,@par_status_akt_value) = 0 THEN 1 ELSE 0 END) ELSE (CASE WHEN CHARINDEX(r.status,@par_status_akt_value) = 0 THEN 0 ELSE 1 END) END) ELSE 1 END)
