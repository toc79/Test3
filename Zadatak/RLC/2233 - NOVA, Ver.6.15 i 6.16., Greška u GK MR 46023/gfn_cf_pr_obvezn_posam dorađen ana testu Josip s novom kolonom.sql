-- USE [NOVA_TEST]
-- GO
-- /****** Object:  UserDefinedFunction [dbo].[gfn_cf_pr_obvezn_posam]    Script Date: 10.2.2021. 9:09:52 ******/
-- SET ANSI_NULLS ON
-- GO
...
-- 22.05.2020 MatjazB; MID 90493 - added RKP
-- 27.05.2020 KlemenV; MID 90335 - added orig_val
-- 08.12.2020 Thor; BID 38561 - Changed znesek_val to znesek_dom in RKP
-- 09.02.2021 Josip; removed temp table @tabela_rkp and union @tabela_rkp
------------------------------------------------------------------------------------------------------------------------------------------------------
...
SELECT 
    ...
    0 as davek,
    p.id_kredpog,
	pg.ID_TEC as orig_tec,
	t.id_val as orig_val,
...
FROM 
	...
	INNER JOIN dbo.TECAJNIC t on t.id_tec = pg.ID_TEC
WHERE 
...
SELECT 	
    ...
    '' as njih_stevilka,
    '' as id_kredpog,
	ISNULL(s.ID_TEC,'') as orig_tec,
	ISNULL(t.id_val,'') as orig_val,
    sum(s.znes_o) as orig_razd,
    0 as orig_obresti,
    '' as oznaka,
    '' as aneks,
    s.id_str as id_str, 
    null as id_rtip
FROM dbo.stroski s
LEFT OUTER JOIN dbo.TECAJNIC t on t.id_tec = s.ID_TEC
...
GROUP BY a.datum, a.leto, a.kvart, a.mesec , a.teden, a.id_tec, a.id_kredpog, a.sifra_kupca, a.kreditodajalec, 
		a.njih_stevilka, a.Id_KredPog, a.orig_tec, a.orig_val, a.oznaka, a.aneks,opis, b.naziv
