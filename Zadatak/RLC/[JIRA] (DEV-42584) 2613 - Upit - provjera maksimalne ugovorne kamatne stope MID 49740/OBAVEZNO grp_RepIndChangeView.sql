PRINT 'Upgrading grp_RepIndChangeView'
IF EXISTS(SELECT * FROM sys.procedures WHERE name = 'grp_RepIndChangeView') DROP PROCEDURE [dbo].[grp_RepIndChangeView]
GO

-- USE [NOVA_INT]
-- GO
/****** Object:  StoredProcedure [dbo].[grp_RepIndChangeView]    Script Date: 4/23/2024 2:38:22 PM ******/
SET ANSI_NULLS ON
GO
...
-- 06.03.2024 MatjazB; TID 32427 - added a.max_ir_used
...
BEGIN
	...
	select 
		...
		case 
            when b.je_nk = 1 or a.installment_credit = 1 then cast(0 as bit)
            when a.fix_dat_rpg = 1 and iobr.znesek <= IIF(x.st_obrok = 0, 0, @toleranca_za_obresti / cast(x.st_obrok as decimal(18,2))) then cast(1 as bit)
            when a.fix_dat_rpg = 0 and iobr.znesek = 0 and a.max_ir_used = 1 then cast(1 as bit)
            else cast(0 as bit)
    ...
    where
        ...
        (
            ...
            or (iobr.znesek = 0 and a.max_ir_used = 1 and a.id_tiprep = 2) -- če se upošteva maksimalna OM in se posledično obrok ni spremenil, prišlo pa je do spremembe indeksa
        )
END
