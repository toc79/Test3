...
--select id_tec, STATUS_AKT, nacin_leas, * from dbo.pogodba where ID_POG='67169/22'
--select top 10 id_tec, STATUS_AKT, * from dbo.pogodba where id_tec = '017' and STATUS_AKT = 'A' and nacin_leas = 'F1' order by id_cont asc
select * from dbo.tecajnic -- TEČAJEVI SU ISTI TAKO DA MOŽE IĆI ISTI IZNOS, TESTIRAO I KOD NJIH

--SQL
declare @id_kredpog varchar(100) = '0274 23'
--select * From dbo.KRED_POG where ID_KREDPOG = @id_kredpog

--644 zapisa
select 
	pog.id_pog
	, pog.net_nal_zac as iznos_akolacije
	, pog.id_cont
	, kp.id_kredpog
	...
	--, pkp.*
	--, pog.id_val
from dbo.POGODBA pog --on dbo.gfn_Id_cont4Id_pog(a.id_pog) = pog.id_cont
left join dbo.pogodba_kp_npm pkp on pog.ID_CONT = pkp.id_cont
inner join dbo._ugovori_za_alociranje_50673 uza on pog.id_cont = uza.id_cont
cross join dbo.KRED_POG kp --on kp.id_kred_pog 
-- dbo.kred_pog_pogodba_allocation a  
where kp.ID_KREDPOG = @id_kredpog 
-- provjere
--and uza.iznos_alokacije = pog.NET_NAL_zac --SVI
and pkp.id_cont is null -- ne smije biti alokacije NEMA
--and pog.NET_NAL = pog.net_nal_zac  --nisu svi ali to nema veze
--and pog.id_val != 'eur' -- nema različitih od 
