podešeno na produkciju. Imaju krediten ugovore koji imaju zapis glavnice i kamate 0, za takve se nije prikazao datum. Eventualno da se koristi uvjet znes_r = 0 za zapis kamate...
rac_obresti je 0 kao i za glavnicu pa ne može po tome. 

--Poročilo o kreditnih pogodb za RAIFFEISEN
--27.01.2021 g_tomislav MR 46176 - dodavanje podatka datuma sljedeće kamate 
...
SELECT  a.id_kredpog, 
  ...
  SELECT  min(dat_zap) as dat_otpl_prve_rate_gl, max(dat_zap) as dat_otpl_zadnje_rate_gl, p.id_kredpog, p.id_oc_report
	...
	SELECT min(dat_zap) datum_otpl_sled_kamate, max(dat_zap) datum_otpl_zadnje_kamate, p.id_kredpog, p.id_oc_report
	INTO #kred_dat_znes_o
	FROM dbo.kred_planp p
	WHERE p.znes_o<>0 AND p.dat_zap > @date_to and p.id_oc_report = @id_oc_report
	GROUP BY p.id_kredpog, p.id_oc_report
