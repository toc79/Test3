 ...
 * begin tran
 * update dokument set OPOMBE= 'test napomena' where id_dokum in( 51305)
 * select OPOMBE, * from dokument where id_dokum in( 51305)
 * --rollback
 
 
 

GF_SQLEXEC(" SELECT CAST(d.sys_ts as bigint) as cast_sys_ts, cast('20200611' as datetime) as vrnjen_new, 'g_tomislav' as lcPopravil, d.* from dbo.dokument d WHERE id_dokum in (51303,51304,51306,52625,51305) order by id_dokum", "_stari_okvir_dokument")
*--d.id_cont = 8765
*fox
