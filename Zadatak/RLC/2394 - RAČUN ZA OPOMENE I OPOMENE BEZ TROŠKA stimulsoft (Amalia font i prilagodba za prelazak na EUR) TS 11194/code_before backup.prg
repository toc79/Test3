...
DO case
	...
	CASE lnOdg = 1	&& Vse
		** zadnji označen zapis ne uvažava pa su potrebne sljedeće 3 linije
		...
		SCAN FOR oznacen = .t.
		ENDSCAN
		Select * From za_opom Where &lcFilter and !GF_NULLOREMPTY(dok_opom) and oznacen = .t. and !(INLIST(id_za_opom_type, 6, 8)) and (lcDom_valuta == "EUR" AND datum_dok > ldMigrationDatePS) INTO CURSOR rezultat
	OTHERWISE
		RETURN .F.
ENDCASE
