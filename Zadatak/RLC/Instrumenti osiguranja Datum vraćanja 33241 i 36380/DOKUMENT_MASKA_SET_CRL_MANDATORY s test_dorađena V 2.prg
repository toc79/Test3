...
IF loForm.tip_vnosne_maske = 2 AND lnDokJeNaListi > 0 THEN 
	...
	* Lista ugovora
	...
	IF !GF_NULLOREMPTY(dokument.id_frame)
		GF_SQLEXEC("SELECT f.id_cont FROM dbo.frame_list a LEFT JOIN dbo.frame_pogodba f ON a.id_frame = f.id_frame WHERE a.status_akt='Z' AND a.id_frame="+gf_quotedstr(dokument.id_frame), "_ef_frame_pogodba")
