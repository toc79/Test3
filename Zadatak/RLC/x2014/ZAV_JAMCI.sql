Declare @emso as varchar(25), @dav_stev as varchar(25), @naz_kr_kup as varchar(80), @ulica_sed as varchar(70), @vr_osebe as char(2)
...
Select CASE WHEN vr_osebe <> 'FO' THEN 'MB: '+ p.emso ELSE 'JMBG: '+ p.emso END as emso, p.dav_stev, p.naz_kr_kup, p.ulica_sed, p.vr_osebe, p.id_poste_sed, 
...
open tmp_j
Fetch next from tmp_j into @emso, @dav_stev, @naz_kr_kup, @ulica_sed, @vr_osebe, @id_poste_sed, @mesto_sed, @direktor, @stev_reg, @dat_sklen
...
CASE WHEN @vr_osebe='SP' THEN ', MBO: '+LTRIM(RTRIM(@stev_reg)) ELSE ', '+LTRIM(RTRIM(@emso)) END +
