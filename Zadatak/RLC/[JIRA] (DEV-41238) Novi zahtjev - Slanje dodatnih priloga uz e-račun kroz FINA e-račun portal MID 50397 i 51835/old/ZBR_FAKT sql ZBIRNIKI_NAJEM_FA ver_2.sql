DECLARE  @tip_leas char(2) --@datum datetime,
--SET @datum = getdate()
...
SELECT 
	...
	, case when rep_ind.id_rep_ind is not null and par.ident_stevilka is not null and par.ident_stevilka <> '' and v.sif_terj = 'LOBR' then 1 else 0 end as print_rep_ind	
...
INNER JOIN dbo.najem_fa a on zn.ST_DOK = a.ST_DOK
...
INNER JOIN dbo.dav_stop h ON ra.id_dav_st = h.id_dav_st
