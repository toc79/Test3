-- 26.10.2023 g_tomislav MID 50397 - created automatic rendering OBV_IND_SSOFT_RLC for not printed after FAK_LOBR_SSOFT_RLC and ZBR_FAKT_SSOFT_RLC for LOBR and FINA partners
...
begin 
	...
	begin
		...
		from (
			...
			where @rendered_report_name in ('FAK_LOBR_SSOFT_RLC') 
			...
			from dbo.najem_fa nf 
			inner join dbo.vrst_ter vt on nf.id_terj = vt.id_terj
			where @rendered_report_name in ('ZBR_FAKT_SSOFT_RLC')
			and vt.sif_terj = 'LOBR'			
			and exists (select * from dbo.zbirniki where id_zbirnik = try_convert(int, @id_rendered) and ddv_id = nf.ddv_id)
			group by nf.id_cont, nf.id_kupca, nf.ddv_id
		) nf
		...
		cross apply (select ri2.max_id_rep_ind, ri1.opombe 
					...
					cross apply (select max(id_rep_ind) as max_id_rep_ind 
								from dbo.rep_ind 
								where izpisan = 0 
								and ddv_date > '20230630' and id_cont = nf.id_cont
							) ri2
					where ri1.id_rep_ind = ri2.max_id_rep_ind
					) as rep_ind
