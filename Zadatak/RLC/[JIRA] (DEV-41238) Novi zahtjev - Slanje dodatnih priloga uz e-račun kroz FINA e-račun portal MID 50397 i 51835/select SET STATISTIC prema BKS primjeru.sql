declare @id varchar(100) = '20230067431'
...
SELECT rep_ind.ID_REP_IND, rep_ind.datum, rep_ind.nobrok, rep_ind.sobrok, p.id_pog, rep_ind.NDATUM, rep_ind.NINDEKS
	, rep_ind.id_cont
INTO #REP_IND
FROM dbo.rep_ind
...
CASE WHEN a.id_kupca IN (select id_kupca
	...
	, dok_SE.STEVILKA
    , trr.trr
	, IsNULL(rind.id_rep_ind,'') test_is_null
	, IsNULL(rind.id_cont,'')  as test_id_cont_is_null
	, rind.ndatum as dat_ind, rind.nindeks as vr_ind,
	CASE WHEN IsNULL(rind.id_rep_ind,'') <> '' THEN 1 ELSE 0 END print_RInd,
