...
CASE WHEN a.id_kupca IN (select id_kupca
	...
	, dok_SE.STEVILKA
    , trr.trr
	-- Obavijest o promjeni indeksa REP_IND stranica
	, rep_ind.id_rep_ind
	, CASE WHEN rep_ind.id_rep_ind is not null THEN 1 ELSE 0 END print_rep_ind
	, rep_ind.ndatum
	, rep_ind.nindeks
	, COALESCE(rep_ind.nobrok, CAST(0.00 AS DECIMAL(18,2))) as nobrok
	, COALESCE(rep_ind.sobrok, CAST(0.00 AS DECIMAL(18,2))) as sobrok
	, rep_ind.datum as rep_ind_datum
	, rep_ind.id_val as rep_ind_id_val
	, rep_ind.naziv_ind
...
LEFT JOIN (select ID_CONT,ID_OBL_ZAV, STEVILKA from dokument where ID_OBL_ZAV='SE' and STATUS_AKT='A' ) dok_SE on dok_SE.id_cont=b.id_cont
LEFT JOIN (select id_kupca, trr from PARTNER_TRR where prioriteta=1) trr on trr.id_kupca=ra.ID_KUPCA
OUTER APPLY (select max(dat_izpisk) as dat_izpisk from dbo.placila where id_app_pren is not null) datIzv
outer apply (
	SELECT rep_ind.id_rep_ind, rep_ind_id rep_ind.datum, rep_ind.nobrok, rep_ind.sobrok, rep_ind.NDATUM, rep_ind.NINDEKS
		, CASE WHEN T.id_val = 'HRK' THEN 'KN' ELSE T.id_val END AS id_val
		, rtip.naziv as naziv_ind 
	FROM dbo.rep_ind
	INNER JOIN dbo.tecajnic T ON T.id_tec = dbo.gfn_GetNewTec(rep_ind.id_tec)
	INNER JOIN dbo.rtip rtip ON rtip.id_rtip = rep_ind.id_rtip
	--INNER JOIN dbo.pogodba p on rep_ind.id_cont = p.id_cont
	WHERE id_rep_ind in (Select max(id_rep_ind) as id_rep_ind From dbo.rep_ind where /*izpisan = 0 and rind_datum > '20230630' and*/ id_cont = a.id_cont /*group by id_cont*/)
	-- ili top 1 order by ili LIMIT 1 ili samo dodati zaseban data set REP_IND pa ako nema data u data bandu, neće se stranici niti ispisivati??
	) as rep_ind
WHERE a.ddv_id = @id