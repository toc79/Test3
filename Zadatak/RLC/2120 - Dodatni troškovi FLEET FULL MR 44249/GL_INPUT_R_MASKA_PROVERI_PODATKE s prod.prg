...
IF RECCOUNT("_DodStrChk") > 0 THEN
 &&- konto dodatnog troška, a nije odabran tip povezanoga dokumenta "Faktura za Full leasing"
 ...
 &&- konto dodatnog troška, a dodatni trošak nije evidentiran na stavku
  SELE _DodStrChk
  GO TOP
  SCAN FOR GF_NULLOREMPTY(id_vrst_dod_str) AND gl_input_r.vrsta_rac!='FLE'
   POZOR("Za stavke koje se kniže na konto "+ALLT(protikonto)+" potrebno je evidentirati vrstu dodatnog troška!")
   REPLACE ni_napaka WITH .F. IN cur_extfunc_error
   RETURN
  ENDSCAN
  
 &&- više stavaka na isti konto s istim dodatnim troškom
  SELE _DodStrChk
  GO TOP 
  SCAN FOR cnt > 1
