...
ENDIF
*******************************************************
** 30.06.2021 g_tomislav MID 47163 - created;

lcGrupa = allt(GF_LOOKUP("vrst_opr.id_grupe", ponudba.id_vrste, "vrst_opr.id_vrste"))
lnCena_dkm = loForm.pgfSve.pgPon.pgfPon.pgOsn.txtCena_dkm.Value

IF RF_TIP_POG(ponudba.nacin_leas) == "OL" AND ponudba.nacin_leas != "OF" AND INLIST(lcGrupa, "VNC", "VLT", "VUC") 
	lnCena_dkm = loForm.pgfSve.pgPon.pgfPon.pgOsn.txtCena_dkm.Value
	lnCijenaDodatnogKM = ROUND((ponudba.vr_val * 0.004) / 1000 , 2)
	
	IF lnCena_dkm = 0 OR lnCena_dkm != lnCijenaDodatnogKM
