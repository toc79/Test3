** ??? - Created 
** 06.08.2021 g_tomislav MID 47311 - bugfix: txtst_fakture and txtnabav_vred can not be null
**TODO
** 09.09.20221 g_tomislav MID 47311 - added ROUND on txtstopnja_am
* DODATI 	lnAmInt = lnznesek/iif(lnNabVred=0, 1, lnNabVred)*100  DJELJENJE S NULOM
...
SELECT A.ID_CONT, A.ID_VRSTE, A.ID_KUPCA, A.TRAJ_NAJ, 
dbo.GFN_XCHANGE('000', A.OPCIJA, A.ID_TEC, A.DAT_SKLEN) AS OPCIJA1, dbo.GFN_XCHANGE('000',A.VARSCINA,A.ID_TEC,A.DAT_SKLEN) AS VARSCINA1,
CAST(A.OPCIJA*A.PO_TECAJU AS DECIMAL (18,2)) AS OPCIJA2, CAST(ISNULL(PP.NETO,0)*A.PO_TECAJU AS DECIMAL(18,2)) AS OPCIJA3,
CAST(A.VARSCINA*A.PO_TECAJU AS DECIMAL(18,2)) AS VARSCINA2,
B.ID_GRUPE, B.NAZIV, C.NAZ_KR_KUP, D.ID_NOMEN AS NOMENKLATURA, 
E.DDV_ID, E.KREDIT_NETO AS KREDIT_NETO, E.DATUM, ISNULL(F.KONTO,'99999') AS KONTO, ISNULL(G.ID_KNJIZBE,'XXX') AS ID_KNJIZBE,
CASE 
...
LEFT JOIN (Select id_cont, neto, robresti From PLANP a
