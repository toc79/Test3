...
** 15.10.2021 g_tomislav MID 47311 - added ROUND on txtstopnja_am. Added calculation of KREDIT_NETO for OF leasing type. Added order by e.id_rac_in
** 12.11.2021 g_tomislav MID 47778 - corrected calculation of KREDIT_NETO for OF leasing type and for take over contracts 
** 16.11.2021 g_tomislav MID 47778 - added calculation of KREDIT_NETO for take over contracts for OL leasing type 
...
SELECT A.ID_CONT, --A.ID_VRSTE, A.ID_KUPCA, 
	...
	E.DDV_ID, E.DATUM, 
	case when a.nacin_leas = 'OF' and (a.prevzeta != '' or E.KREDIT_NETO is null) then isnull(of1.sum_kredit_dom, 0) 
		when dbo.gfn_Nacin_leas_HR(a.nacin_leas) = 'OL' and a.prevzeta != '' then ol1.sum_debit_dom
		else E.KREDIT_NETO end AS KREDIT_NETO, 
...
LEFT JOIN dbo.FA_NOMENKL D ON B.ID_GRUPE = D.OZNAKA1
LEFT JOIN dbo.RAC_IN E ON A.ID_CONT = E.ID_CONT
LEFT JOIN dbo.GL F ON E.DDV_ID = F.ST_DOK AND E.KREDIT_NETO = F.DEBIT_DOM
LEFT JOIN dbo.FA_KONTI_VK G ON F.KONTO = G.KONTO
LEFT JOIN (Select id_cont, neto, robresti 
		From dbo.PLANP a
		INNER JOIN dbo.VRST_TER VT ON a.ID_TERJ = VT.id_terj AND VT.sif_terj = 'OPC') PP ON a.ID_CONT = PP.ID_CONT
